...
try {
    ...
    $res = $dbh->prepare('SELECT `id_vid`, p.`fam`,p.`name`,p.`otchestvo`, g.`name_group`, b.`name_book`,b.`year_create`, `data_vidachi`, `data_vozvrata`, `na_rukah`, `poterya`, `primechanie` FROM `vidacha` v JOIN `people` p ON v.`id_man`=p.`id_man` JOIN `student` s ON s.`id_man`=p.`id_man` JOIN `group` g ON s.`id_group`=g.`id_group` JOIN `book` b ON v.`id_book`=b.`id_book` LEFT JOIN `mail` m ON v.`id_man`=m.`id_man` WHERE b.`id_kafedra`=? AND m.`id_man` IS NULL AND v.`na_rukah`="Yes" AND v.`poterya`="No" AND ((YEAR(CURDATE())-YEAR(data_vidachi)>=1) OR (MONTH(data_vidachi)<=11 AND YEAR(CURDATE())-YEAR(`data_vidachi`)=1 AND MONTH(CURDATE())>5) OR (YEAR(CURDATE())-YEAR(data_vidachi)=0 AND MONTH(CURDATE())>6 AND MONTH(`data_vidachi`)<=5))'.$qWhere.' ORDER BY '.$sortingField.' '.$sortingOrder.' LIMIT '.$firstRowIndex.', '.$rowsPerPage);
    ...
    while($row = $res->fetch(PDO::FETCH_ASSOC)) {
        ...
        $response->rows[$i]['cell']=array($row['id_vid'],$row['fam'],$row['name'],$row['otchestvo'],$row['name_group'], $row['name_book'],$row['year_create']);
		      $i++;
    }
