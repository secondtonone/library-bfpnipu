...
if ($_POST['oper']=="edit"){
	...
	$query=$dbh->prepare('UPDATE `student` SET `number_zach`=?, `id_group`=?, `god_postup`=?,`otchislen_iz_group`=?,`data_change`=NOW() WHERE `id_man`=?');
    $query->execute(array($_POST['number_zach'],$_POST['id_group'],$_POST['god_postup'],$_POST['otchislen_iz_group'],$_POST['id']));
...
if ($_POST['oper']=="add")
{
 $query=$dbh->prepare('INSERT INTO `people` (`fam`, `name`, `otchestvo`,`telefon_dom`, `telefon_sot`, `e_mail`, `mesto_raboti`, `ceh_otdel`, `doljnost`, `telefon_rabochii`,`data_change`,`data_zapoln_anketi`) VALUES (?,?,?,?,?,?,?,?,?,?,NOW(),NOW())');
	...
	$query=$dbh->prepare('INSERT INTO `student` (`id_man`,`number_zach`, `id_group`, `god_postup`,`otchislen_iz_group`,`data_change`) VALUES (?,?,?,?,NOW())');
    $query->execute(array($lastid,$_POST['number_zach'],$_POST['id_group'],$_POST['god_postup'],$_POST['otchislen_iz_group']));
