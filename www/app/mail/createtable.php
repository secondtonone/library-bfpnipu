...
while($row = $query->fetch(PDO::FETCH_ASSOC)) 
	...
	$res = $dbh->prepare('SELECT `name_book` FROM `book` as b INNER JOIN `vidacha` as v  ON v.`id_book`=b.`id_book` INNER JOIN `users` as u ON v.`id_man`=u.`id_man` WHERE `na_rukah`="Yes" AND `poterya`="No" AND ((YEAR(CURDATE())-YEAR(data_vidachi)>=1) OR (YEAR(CURDATE())-YEAR(`data_vidachi`)=1) OR (YEAR(CURDATE())-YEAR(data_vidachi)=0 AND MONTH(CURDATE())>6 AND MONTH(`data_vidachi`)<=5)) AND v.id_man=?');
