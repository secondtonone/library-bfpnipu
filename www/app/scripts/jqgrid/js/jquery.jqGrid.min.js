 ...
 Date: 2013-05-13
*/
(function(a){a.jgrid=a.jgrid||{};a.extend(a.jgrid,{version:"4.5.0",htmlDecode:function(a){return a&&("&nbsp;"===a||"&#160;"===a||1===a.length&&160===a.charCodeAt(0))?"":!a?a:(""+a).replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&quot;/g,'"').replace(/&amp;/g,"&")},htmlEncode:function(a){return!a?a:(""+a).replace(/&/g,"&amp;").replace(/\"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")},format:function(c){var e=a.makeArray(arguments).slice(1);null==c&&(c="");return c.replace(/\{(\d+)\}/g,
...
(c=60*Number(h[5])+Number(h[6]),c*="-"===h[4]?1:-1,c-=i.getTimezoneOffset(),i.setTime(Number(Number(i)+6E4*c)));else{e=(""+e).replace(/\\T/g,"T").replace(/\\t/,"t").split(f.parseRe);c=c.replace(/\\T/g,"T").replace(/\\t/,"t").split(f.parseRe);j=0;for(k=c.length;j<k;j++)"M"===c[j]&&(h=a.inArray(e[j],f.monthNames),-1!==h&&12>h&&(e[j]=h+1,g.m=e[j])),"F"===c[j]&&(h=a.inArray(e[j],f.monthNames),-1!==h&&11<h&&(e[j]=h+1-12,g.m=e[j])),"a"===c[j]&&(h=a.inArray(e[j],f.AmPm),-1!==h&&2>h&&e[j]===f.AmPm[h]&&(e[j]=
h,g.h=i(e[j],g.h))),"A"===c[j]&&(h=a.inArray(e[j],f.AmPm),-1!==h&&1<h&&e[j]===f.AmPm[h]&&(e[j]=h-2,g.h=i(e[j],g.h))),"g"===c[j]&&(g.h=parseInt(e[j],10)),void 0!==e[j]&&(g[c[j].toLowerCase()]=parseInt(e[j],10));g.f&&(g.m=g.f);if(0===g.m&&0===g.y&&0===g.d)return"&#160;";g.m=parseInt(g.m,10)-1;i=g.y;70<=i&&99>=i?g.y=1900+g.y:0<=i&&69>=i&&(g.y=2E3+g.y);i=new Date(g.y,g.m,g.d,g.h,g.i,g.s,g.u)}else i=new Date(g.y,g.m,g.d,g.h,g.i,g.s,g.u);if(void 0===d)return i;f.masks.hasOwnProperty(d)?d=f.masks[d]:d||
(d="Y-m-d");c=i.getHours();e=i.getMinutes();g=i.getDate();h=i.getMonth()+1;j=i.getTimezoneOffset();k=i.getSeconds();var m=i.getMilliseconds(),n=i.getDay(),o=i.getFullYear(),l=(n+6)%7+1,p=(new Date(o,h-1,g)-new Date(o,0,1))/864E5,q={d:b(g),D:f.dayNames[n],j:g,l:f.dayNames[n+7],N:l,S:f.S(g),w:n,z:p,W:5>l?Math.floor((p+l-1)/7)+1:Math.floor((p+l-1)/7)||(4>((new Date(o-1,0,1)).getDay()+6)%7?53:52),F:f.monthNames[h-1+12],m:b(h),M:f.monthNames[h-1],n:h,t:"?",L:"?",o:"?",Y:o,y:(""+o).substring(2),a:12>c?
f.AmPm[0]:f.AmPm[1],A:12>c?f.AmPm[2]:f.AmPm[3],B:"?",g:c%12||12,G:c,h:b(c%12||12),H:b(c),i:b(e),s:b(k),u:m,e:"?",I:"?",O:(0<j?"-":"+")+b(100*Math.floor(Math.abs(j)/60)+Math.abs(j)%60,4),P:"?",T:((""+i).match(/\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g)||[""]).pop().replace(/[^-+\dA-Z]/g,""),Z:"?",c:"?",r:"?",U:Math.floor(i/1E3)};return d.replace(/\\.|[dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU]/g,function(a){return q.hasOwnProperty(a)?
...
10),L=b.p.scroll?a.jgrid.randId():1;if(F>0&&b.p.page<=0)b.p.page=1;if(d&&F){g&&(P=P*(g+1));var g=a.isFunction(b.p.afterInsertRow),S=false,X;if(b.p.grouping){S=b.p.groupingView.groupCollapse===true;X=a.jgrid.getMethod("groupingPrepare")}for(;y<F;){G=d[y];E=l(G,L+y);E=b.p.idPrefix+E;k=e===0?0:e+1;Q=(k+y)%2===1?D:"";var K=M.length;M.push("");H&&M.push(s(0,y,b.p.page,b.p.rowNum));x&&M.push(r(E,H,y,false));C&&M.push(z.call(m,x+H,y+e));if(j.repeatitems){I||(I=v(x+C+H));var O=a.jgrid.getXmlData(G,j.cell,
true);a.each(I,function(a){var d=O[this];if(!d)return false;p=d.textContent||d.text;A[b.p.colModel[a+x+C+H].name]=p;M.push(q(E,p,a+x+C+H,y+e,G,A))})}else for(k=0;k<B.length;k++){p=a.jgrid.getXmlData(G,B[k]);A[b.p.colModel[k+x+C+H].name]=p;M.push(q(E,p,k+x+C+H,y+e,G,A))}M[K]=J(E,S,Q,A,G,false);M.push("</tr>");if(b.p.grouping){N=X.call(m,M,N,A,y);M=[]}if(h||b.p.treeGrid===true){A._id_=a.jgrid.stripPref(b.p.idPrefix,E);b.p.data.push(A);b.p._index[A._id_]=b.p.data.length-1}if(b.p.gridview===false){a("tbody:first",
c).append(M.join(""));m.triggerHandler("jqGridAfterInsertRow",[E,A,G]);g&&b.p.afterInsertRow.call(b,E,A,G);M=[]}A={};n++;y++;if(n===P)break}}if(b.p.gridview===true){o=b.p.treeANode>-1?b.p.treeANode:0;if(b.p.grouping){m.jqGrid("groupingRender",N,b.p.colModel.length);N=null}else b.p.treeGrid===true&&o>0?a(b.rows[o]).after(M.join("")):a("tbody:first",c).append(M.join(""))}if(b.p.subGrid===true)try{m.jqGrid("addSubGrid",x+H)}catch(T){}b.p.totaltime=new Date-i;if(n>0&&b.p.records===0)b.p.records=F;M=null;
if(b.p.treeGrid===true)try{m.jqGrid("setTreeNode",o+1,n+o+1)}catch(V){}if(!b.p.treeGrid&&!b.p.scroll)b.grid.bDiv.scrollTop=0;b.p.reccount=n;b.p.treeANode=-1;b.p.userDataOnFooter&&m.jqGrid("footerData","set",b.p.userData,true);if(h){b.p.records=F;b.p.lastpage=Math.ceil(F/P)}f||b.updatepager(false,true);if(h)for(;n<F;){G=d[n];E=l(G,n+L);E=b.p.idPrefix+E;if(j.repeatitems){I||(I=v(x+C+H));var $=a.jgrid.getXmlData(G,j.cell,true);a.each(I,function(a){var d=$[this];if(!d)return false;p=d.textContent||d.text;
...
"local"){c=b.p.localReader;h="local"}else{c=b.p.jsonReader;h="json"}var l=a(b),k=0,m,o,n,p=[],x=b.p.multiselect?1:0,C=b.p.subGrid===true?1:0,z,H=b.p.rownumbers===true?1:0,u=v(x+C+H);h=t(h);var B,A,E,G={},I,y,F=[],D=b.p.altRows===true?b.p.altclass:"",Q;b.p.page=a.jgrid.getAccessor(d,c.page)||b.p.page||0;A=a.jgrid.getAccessor(d,c.total);C&&(z=a.jgrid.getMethod("addSubGridCell"));b.p.lastpage=A===void 0?1:A;b.p.records=a.jgrid.getAccessor(d,c.records)||0;b.p.userData=a.jgrid.getAccessor(d,c.userdata)||
{};E=b.p.keyIndex===false?a.isFunction(c.id)?c.id.call(b,d):c.id:b.p.keyIndex;if(!c.repeatitems){p=h;if(p.length>0&&!isNaN(E)){b.p.remapColumns&&b.p.remapColumns.length&&(E=a.inArray(E,b.p.remapColumns));E=p[E]}}A=a.jgrid.getAccessor(d,c.root);A==null&&a.isArray(d)&&(A=d);A||(A=[]);d=A.length;o=0;if(d>0&&b.p.page<=0)b.p.page=1;var N=parseInt(b.p.rowNum,10),P=b.p.scroll?a.jgrid.randId():1,L=false,S;g&&(N=N*(g+1));b.p.datatype==="local"&&!b.p.deselectAfterSort&&(L=true);var X=a.isFunction(b.p.afterInsertRow),
K=[],O=false,T;if(b.p.grouping){O=b.p.groupingView.groupCollapse===true;T=a.jgrid.getMethod("groupingPrepare")}for(;o<d;){g=A[o];y=a.jgrid.getAccessor(g,E);if(y===void 0){typeof E==="number"&&b.p.colModel[E+x+C+H]!=null&&(y=a.jgrid.getAccessor(g,b.p.colModel[E+x+C+H].name));if(y===void 0){y=P+o;if(p.length===0&&c.cell){m=a.jgrid.getAccessor(g,c.cell)||g;y=m!=null&&m[E]!==void 0?m[E]:y}}}y=b.p.idPrefix+y;m=e===1?0:e;Q=(m+o)%2===1?D:"";L&&(S=b.p.multiselect?a.inArray(y,b.p.selarrrow)!==-1:y===b.p.selrow);
var $=F.length;F.push("");H&&F.push(s(0,o,b.p.page,b.p.rowNum));x&&F.push(r(y,H,o,S));C&&F.push(z.call(l,x+H,o+e));B=h;if(c.repeatitems){c.cell&&(g=a.jgrid.getAccessor(g,c.cell)||g);a.isArray(g)&&(B=u)}for(n=0;n<B.length;n++){m=a.jgrid.getAccessor(g,B[n]);G[b.p.colModel[n+x+C+H].name]=m;F.push(q(y,m,n+x+C+H,o+e,g,G))}F[$]=J(y,O,Q,G,g,S);F.push("</tr>");if(b.p.grouping){K=T.call(l,F,K,G,o);F=[]}if(j||b.p.treeGrid===true){G._id_=a.jgrid.stripPref(b.p.idPrefix,y);b.p.data.push(G);b.p._index[G._id_]=
b.p.data.length-1}if(b.p.gridview===false){a("#"+a.jgrid.jqID(b.p.id)+" tbody:first").append(F.join(""));l.triggerHandler("jqGridAfterInsertRow",[y,G,g]);X&&b.p.afterInsertRow.call(b,y,G,g);F=[]}G={};k++;o++;if(k===N)break}if(b.p.gridview===true){I=b.p.treeANode>-1?b.p.treeANode:0;b.p.grouping?l.jqGrid("groupingRender",K,b.p.colModel.length):b.p.treeGrid===true&&I>0?a(b.rows[I]).after(F.join("")):a("#"+a.jgrid.jqID(b.p.id)+" tbody:first").append(F.join(""))}if(b.p.subGrid===true)try{l.jqGrid("addSubGrid",
x+H)}catch(V){}b.p.totaltime=new Date-i;if(k>0&&b.p.records===0)b.p.records=d;if(b.p.treeGrid===true)try{l.jqGrid("setTreeNode",I+1,k+I+1)}catch(R){}if(!b.p.treeGrid&&!b.p.scroll)b.grid.bDiv.scrollTop=0;b.p.reccount=k;b.p.treeANode=-1;b.p.userDataOnFooter&&l.jqGrid("footerData","set",b.p.userData,true);if(j){b.p.records=d;b.p.lastpage=Math.ceil(d/N)}f||b.updatepager(false,true);if(j)for(;k<d&&A[k];){g=A[k];y=a.jgrid.getAccessor(g,E);if(y===void 0){typeof E==="number"&&b.p.colModel[E+x+C+H]!=null&&
(y=a.jgrid.getAccessor(g,b.p.colModel[E+x+C+H].name));if(y===void 0){y=P+k;if(p.length===0&&c.cell){e=a.jgrid.getAccessor(g,c.cell)||g;y=e!=null&&e[E]!==void 0?e[E]:y}}}if(g){y=b.p.idPrefix+y;B=h;if(c.repeatitems){c.cell&&(g=a.jgrid.getAccessor(g,c.cell)||g);a.isArray(g)&&(B=u)}for(n=0;n<B.length;n++)G[b.p.colModel[n+x+C+H].name]=a.jgrid.getAccessor(g,B[n]);G._id_=a.jgrid.stripPref(b.p.idPrefix,y);b.p.data.push(G);b.p._index[G._id_]=b.p.data.length-1;G={}}k++}}},F=function(){function d(a){var b=0,
...
var c=b.p.multiSort?[]:"",e=[],f=false,g={},i=[],h=[],j,l,k;if(a.isArray(b.p.data)){var m=b.p.grouping?b.p.groupingView:false,o,n;a.each(b.p.colModel,function(){l=this.sorttype||"text";if(l==="date"||l==="datetime"){if(this.formatter&&typeof this.formatter==="string"&&this.formatter==="date"){j=this.formatoptions&&this.formatoptions.srcformat?this.formatoptions.srcformat:a.jgrid.formatter.date.srcformat;k=this.formatoptions&&this.formatoptions.newformat?this.formatoptions.newformat:a.jgrid.formatter.date.newformat}else j=
k=this.datefmt||"Y-m-d";g[this.name]={stype:l,srcfmt:j,newfmt:k}}else g[this.name]={stype:l,srcfmt:"",newfmt:""};if(b.p.grouping){n=0;for(o=m.groupField.length;n<o;n++)if(this.name===m.groupField[n]){var d=this.name;if(this.index)d=this.index;i[n]=g[d];h[n]=d}}if(b.p.multiSort){if(this.lso){c.push(this.name);d=this.lso.split("-");e.push(d[d.length-1])}}else if(!f&&(this.index===b.p.sortname||this.name===b.p.sortname)){c=this.name;f=true}});if(b.p.treeGrid)a(b).jqGrid("SortTree",c,b.p.sortorder,g[c].stype,
...
a.andNot().endsWith},ew:function(a){return a.endsWith},ni:function(a,b){return b==="OR"?a.orNot().equals:a.andNot().equals},"in":function(a){return a.equals},nu:function(a){return a.isNull},nn:function(a,b){return b==="OR"?a.orNot().isNull:a.andNot().isNull}},p=a.jgrid.from(b.p.data);b.p.ignoreCase&&(p=p.ignoreCase());if(b.p.search===true){var s=b.p.postData.filters;if(s){typeof s==="string"&&(s=a.jgrid.parse(s));d(s)}else try{p=q[b.p.postData.searchOper](p)(b.p.postData.searchField,b.p.postData.searchString,
g[b.p.postData.searchField])}catch(t){}}if(b.p.grouping)for(n=0;n<o;n++)p.orderBy(h[n],m.groupOrder[n],i[n].stype,i[n].srcfmt);b.p.multiSort?a.each(c,function(a){p.orderBy(this,e[a],g[this].stype,g[this].srcfmt)}):c&&b.p.sortorder&&f&&(b.p.sortorder.toUpperCase()==="DESC"?p.orderBy(b.p.sortname,"d",g[c].stype,g[c].srcfmt):p.orderBy(b.p.sortname,"a",g[c].stype,g[c].srcfmt));var s=p.select(),r=parseInt(b.p.rowNum,10),x=s.length,v=parseInt(b.p.page,10),w=Math.ceil(x/r),C={},s=s.slice((v-1)*r,v*r),g=
p=null;C[b.p.localReader.total]=w;C[b.p.localReader.page]=v;C[b.p.localReader.records]=x;C[b.p.localReader.root]=s;C[b.p.localReader.userdata]=b.p.userData;s=null;return C}}},u=function(){b.grid.hDiv.loading=true;if(!b.p.hiddengrid)switch(b.p.loadui){case "enable":a("#load_"+a.jgrid.jqID(b.p.id)).show();break;case "block":a("#lui_"+a.jgrid.jqID(b.p.id)).show();a("#load_"+a.jgrid.jqID(b.p.id)).show()}},L=function(){b.grid.hDiv.loading=false;switch(b.p.loadui){case "enable":a("#load_"+a.jgrid.jqID(b.p.id)).hide();
...
b.p.rowTotal;var i=a.isFunction(b.p.loadComplete),h=i?b.p.loadComplete:null,j=0,d=d||1;if(d>1)if(g.npage!==null){e[g.npage]=d;j=d-1;d=1}else h=function(a){b.p.page++;b.grid.hDiv.loading=false;i&&b.p.loadComplete.call(b,a);D(d-1)};else g.npage!==null&&delete b.p.postData[g.npage];if(b.p.grouping){a(b).jqGrid("groupingSetup");var l=b.p.groupingView,k,m="";for(k=0;k<l.groupField.length;k++){var n=l.groupField[k];a.each(b.p.colModel,function(a,b){if(b.name===n&&b.index)n=b.index});m=m+(n+" "+l.groupOrder[k]+
...
L()}}}},O=function(d){a("#cb_"+a.jgrid.jqID(b.p.id),b.grid.hDiv)[b.p.useProp?"prop":"attr"]("checked",d);if(b.p.frozenColumns&&b.p.id+"_frozen")a("#cb_"+a.jgrid.jqID(b.p.id),b.grid.fhDiv)[b.p.useProp?"prop":"attr"]("checked",d)},V=function(d,c){var e="",f="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",g="",h,j,l,k,m=function(d){var c;a.isFunction(b.p.onPaging)&&(c=b.p.onPaging.call(b,d));b.p.selrow=null;if(b.p.multiselect){b.p.selarrrow=
[];O(false)}b.p.savedRow=[];return c==="stop"?false:true},d=d.substr(1),c=c+("_"+d);h="pg_"+d;j=d+"_left";l=d+"_center";k=d+"_right";a("#"+a.jgrid.jqID(d)).append("<div id='"+h+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+j+"' align='left'></td><td id='"+l+"' align='center' style='white-space:pre;'></td><td id='"+k+"' align='right'></td></tr></tbody></table></div>").attr("dir",
...
a("#first"+a.jgrid.jqID(c)+", #prev"+a.jgrid.jqID(c)+", #next"+a.jgrid.jqID(c)+", #last"+a.jgrid.jqID(c)).click(function(){var a=n(b.p.page,1),d=n(b.p.lastpage,1),e=false,f=true,g=true,i=true,h=true;if(d===0||d===1)h=i=g=f=false;else if(d>1&&a>=1)if(a===1)g=f=false;else{if(a===d)h=i=false}else if(d>1&&a===0){h=i=false;a=d-1}if(!m(this.id))return false;if(this.id==="first"+c&&f){b.p.page=1;e=true}if(this.id==="prev"+c&&g){b.p.page=a-1;e=true}if(this.id==="next"+c&&i){b.p.page=a+1;e=true}if(this.id===
"last"+c&&h){b.p.page=d;e=true}e&&D();return false})}b.p.pginput===true&&a("input.ui-pg-input","#"+h).keypress(function(d){if((d.charCode||d.keyCode||0)===13){if(!m("user"))return false;a(this).val(n(a(this).val(),1));b.p.page=a(this).val()>0?a(this).val():b.p.page;D();return false}return this})},Y=function(d){var c,e="",f=b.p.colModel,g=false,i=b.grid.headers[d].el,h="";a("span.ui-grid-ico-sort",i).addClass("ui-state-disabled");a(i).attr("aria-selected","false");if(f[d].lso)if(f[d].lso==="asc"){f[d].lso=
f[d].lso+"-desc";h="desc"}else if(f[d].lso==="desc"){f[d].lso=f[d].lso+"-asc";h="asc"}else{if(f[d].lso==="asc-desc"||f[d].lso==="desc-asc")f[d].lso=""}else f[d].lso=h=f.firstsortorder||"asc";if(h){a("span.s-ico",i).show();a("span.ui-icon-"+h,i).removeClass("ui-state-disabled");a(i).attr("aria-selected","true")}else b.p.viewsortcols[0]||a("span.s-ico",i).hide();b.p.sortorder="";a.each(f,function(a){if(this.lso){a>0&&g&&(e=e+", ");c=this.lso.split("-");e=e+(f[a].index||f[a].name);e=e+(" "+c[c.length-
1]);g=true;b.p.sortorder=c[c.length-1]}});d=e.lastIndexOf(b.p.sortorder);e=e.substring(0,d);b.p.sortname=e},W=function(d,c,e,f){if(b.p.colModel[c].sortable){var g;if(!(b.p.savedRow.length>0)){if(!e){if(b.p.lastsort===c)if(b.p.sortorder==="asc")b.p.sortorder="desc";else{if(b.p.sortorder==="desc")b.p.sortorder="asc"}else b.p.sortorder=b.p.colModel[c].firstsortorder||"asc";b.p.page=1}if(b.p.multiSort)Y(c,e,g);else{if(f){if(b.p.lastsort===c&&b.p.sortorder===f&&!e)return;b.p.sortorder=f}e=b.grid.headers[b.p.lastsort].el;
f=b.grid.headers[c].el;a("span.ui-grid-ico-sort",e).addClass("ui-state-disabled");a(e).attr("aria-selected","false");a("span.ui-icon-"+b.p.sortorder,f).removeClass("ui-state-disabled");a(f).attr("aria-selected","true");if(!b.p.viewsortcols[0]&&b.p.lastsort!==c){a("span.s-ico",e).hide();a("span.s-ico",f).show()}d=d.substring(5+b.p.id.length+1);b.p.sortname=b.p.colModel[c].index||d;g=b.p.sortorder}if(a(b).triggerHandler("jqGridSortCol",[d,c,g])==="stop")b.p.lastsort=c;else if(a.isFunction(b.p.onSortCol)&&
b.p.onSortCol.call(b,d,c,g)==="stop")b.p.lastsort=c;else{if(b.p.datatype==="local")b.p.deselectAfterSort&&a(b).jqGrid("resetSelection");else{b.p.selrow=null;b.p.multiselect&&O(false);b.p.selarrrow=[];b.p.savedRow=[]}if(b.p.scroll){e=b.grid.bDiv.scrollLeft;w.call(b,true,false);b.grid.hDiv.scrollLeft=e}b.p.subGrid&&b.p.datatype==="local"&&a("td.sgexpanded","#"+a.jgrid.jqID(b.p.id)).each(function(){a(this).trigger("click")});D();b.p.lastsort=c;if(b.p.sortname!==d&&c)b.p.lastsort=c}}}},ca=function(d){d=
a(b.grid.headers[d].el);d=[d.position().left+d.outerWidth()];b.p.direction==="rtl"&&(d[0]=b.p.width-d[0]);d[0]=d[0]-b.grid.bDiv.scrollLeft;d.push(a(b.grid.hDiv).position().top);d.push(a(b.grid.bDiv).offset().top-a(b.grid.hDiv).offset().top+a(b.grid.bDiv).height());return d},Z=function(d){var c,e=b.grid.headers,f=a.jgrid.getCellIndex(d);for(c=0;c<e.length;c++)if(d===e[c].el){f=c;break}return f};this.p.id=this.id;-1===a.inArray(b.p.multikey,["shiftKey","altKey","ctrlKey"])&&(b.p.multikey=!1);b.p.keyIndex=
!1;for(g=0;g<b.p.colModel.length;g++)b.p.colModel[g]=a.extend(!0,{},b.p.cmTemplate,b.p.colModel[g].template||{},b.p.colModel[g]),!1===b.p.keyIndex&&!0===b.p.colModel[g].key&&(b.p.keyIndex=g);b.p.sortorder=b.p.sortorder.toLowerCase();a.jgrid.cell_width=a.jgrid.cellWidth();!0===b.p.grouping&&(b.p.scroll=!1,b.p.rownumbers=!1,b.p.treeGrid=!1,b.p.gridview=!0);if(!0===this.p.treeGrid){try{a(this).jqGrid("setTreeGrid")}catch(da){}"local"!==b.p.datatype&&(b.p.localReader={id:"_id_"})}if(this.p.subGrid)try{a(b).jqGrid("setSubGrid")}catch(ea){}this.p.multiselect&&
...
cell:"cell",id:"id",userdata:"userdata",subgrid:{root:"rows",repeatitems:!0,cell:"cell"}},b.p.localReader);b.p.scroll&&(b.p.pgbuttons=!1,b.p.pginput=!1,b.p.rowList=[]);b.p.data.length&&z();var C="<thead><tr class='ui-jqgrid-labels' role='rowheader'>",H,I,E,S,N,A,y,Q,$=Q="",P=[],ha=[];I=[];if(!0===b.p.shrinkToFit&&!0===b.p.forceFit)for(g=b.p.colModel.length-1;0<=g;g--)if(!b.p.colModel[g].hidden){b.p.colModel[g].resizable=!1;break}"horizontal"===b.p.viewsortcols[1]&&(Q=" ui-i-asc",$=" ui-i-desc");H=
k?"class='ui-th-div-ie'":"";Q="<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+Q+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+i+"'></span>"+("<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+$+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+i+"'></span></span>");if(b.p.multiSort){P=b.p.sortname.split(",");for(g=0;g<P.length;g++)I=a.trim(P[g]).split(" "),P[g]=a.trim(I[0]),ha[g]=I[1]?a.trim(I[1]):b.p.sortorder||"asc"}for(g=
0;g<this.p.colNames.length;g++)I=b.p.headertitles?' title="'+a.jgrid.stripHtml(b.p.colNames[g])+'"':"",C+="<th id='"+b.p.id+"_"+b.p.colModel[g].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+i+"'"+I+">",I=b.p.colModel[g].index||b.p.colModel[g].name,C+="<div id='jqgh_"+b.p.id+"_"+b.p.colModel[g].name+"' "+H+">"+b.p.colNames[g],b.p.colModel[g].width=b.p.colModel[g].width?parseInt(b.p.colModel[g].width,10):150,"boolean"!==typeof b.p.colModel[g].title&&(b.p.colModel[g].title=
!0),b.p.colModel[g].lso="",I===b.p.sortname&&(b.p.lastsort=g),b.p.multiSort&&(I=a.inArray(I,P),-1!==I&&(b.p.colModel[g].lso=ha[I])),C+=Q+"</div></th>";Q=null;a(this).append(C+"</tr></thead>");a("thead tr:first th",this).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});if(this.p.multiselect){var X=[],aa;a("#cb_"+a.jgrid.jqID(b.p.id),this).bind("click",function(){b.p.selarrrow=[];var d=b.p.frozenColumns===true?b.p.id+"_frozen":"";if(this.checked){a(b.rows).each(function(c){if(c>
...
aa=true;X=[]}else{a(b.rows).each(function(c){if(c>0&&!a(this).hasClass("ui-subgrid")&&!a(this).hasClass("ui-state-disabled")){a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+a.jgrid.jqID(this.id))[b.p.useProp?"prop":"attr"]("checked",false);a(this).removeClass("ui-state-highlight").attr("aria-selected","false");X.push(this.id);if(d){a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+a.jgrid.jqID(this.id),b.grid.fbDiv)[b.p.useProp?"prop":"attr"]("checked",false);a("#"+a.jgrid.jqID(this.id),b.grid.fbDiv).removeClass("ui-state-highlight")}}});
b.p.selrow=null;aa=false}a(b).triggerHandler("jqGridSelectAll",[aa?b.p.selarrrow:X,aa]);a.isFunction(b.p.onSelectAll)&&b.p.onSelectAll.call(b,aa?b.p.selarrrow:X,aa)})}!0===b.p.autowidth&&(C=a(m).innerWidth(),b.p.width=0<C?C:"nw");(function(){var d=0,c=a.jgrid.cell_width?0:n(b.p.cellLayout,0),f=0,g,i=n(b.p.scrollOffset,0),j,l=false,k,m=0,o=0,q;a.each(b.p.colModel,function(){if(this.hidden===void 0)this.hidden=false;if(b.p.grouping&&b.p.autowidth){var e=a.inArray(this.name,b.p.groupingView.groupField);
if(e>=0&&b.p.groupingView.groupColumnShow.length>e)this.hidden=!b.p.groupingView.groupColumnShow[e]}this.widthOrg=j=n(this.width,0);if(this.hidden===false){d=d+(j+c);this.fixed?m=m+(j+c):f++;o++}});if(isNaN(b.p.width))b.p.width=d+(b.p.shrinkToFit===false&&!isNaN(b.p.height)?i:0);e.width=b.p.width;b.p.tblwidth=d;if(b.p.shrinkToFit===false&&b.p.forceFit===true)b.p.forceFit=false;if(b.p.shrinkToFit===true&&f>0){k=e.width-c*f-m;if(!isNaN(b.p.height)){k=k-i;l=true}d=0;a.each(b.p.colModel,function(a){if(this.hidden===
false&&!this.fixed){this.width=j=Math.round(k*this.width/(b.p.tblwidth-c*f-m));d=d+j;g=a}});q=0;l?e.width-m-(d+c*f)!==i&&(q=e.width-m-(d+c*f)-i):!l&&Math.abs(e.width-m-(d+c*f))!==1&&(q=e.width-m-(d+c*f));b.p.colModel[g].width=b.p.colModel[g].width+q;b.p.tblwidth=d+q+c*f+m;if(b.p.tblwidth>b.p.width){b.p.colModel[g].width=b.p.colModel[g].width-(b.p.tblwidth-parseInt(b.p.width,10));b.p.tblwidth=b.p.width}}})();a(m).css("width",e.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+b.p.id+
"'>&#160;</div>");a(j).css("width",e.width+"px");var C=a("thead:first",b).get(0),T="";b.p.footerrow&&(T+="<table role='grid' style='width:"+b.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+i+"'>");var j=a("tr:first",C),ba="<tr class='jqgfirstrow' role='row' style='height:auto'>";b.p.disableClick=!1;a("th",j).each(function(d){E=b.p.colModel[d].width;if(b.p.colModel[d].resizable===void 0)b.p.colModel[d].resizable=
true;if(b.p.colModel[d].resizable){S=document.createElement("span");a(S).html("&#160;").addClass("ui-jqgrid-resize ui-jqgrid-resize-"+i).css("cursor","col-resize");a(this).addClass(b.p.resizeclass)}else S="";a(this).css("width",E+"px").prepend(S);var c="";if(b.p.colModel[d].hidden){a(this).css("display","none");c="display:none;"}ba=ba+("<td role='gridcell' style='height:0px;width:"+E+"px;"+c+"'></td>");e.headers[d]={width:E,el:this};N=b.p.colModel[d].sortable;if(typeof N!=="boolean")N=b.p.colModel[d].sortable=
true;c=b.p.colModel[d].name;c==="cb"||c==="subgrid"||c==="rn"||b.p.viewsortcols[2]&&a(">div",this).addClass("ui-jqgrid-sortable");if(N)if(b.p.multiSort)if(b.p.viewsortcols[0]){a("div span.s-ico",this).show();b.p.colModel[d].lso&&a("div span.ui-icon-"+b.p.colModel[d].lso,this).removeClass("ui-state-disabled")}else{if(b.p.colModel[d].lso){a("div span.s-ico",this).show();a("div span.ui-icon-"+b.p.colModel[d].lso,this).removeClass("ui-state-disabled")}}else if(b.p.viewsortcols[0]){a("div span.s-ico",
this).show();d===b.p.lastsort&&a("div span.ui-icon-"+b.p.sortorder,this).removeClass("ui-state-disabled")}else if(d===b.p.lastsort){a("div span.s-ico",this).show();a("div span.ui-icon-"+b.p.sortorder,this).removeClass("ui-state-disabled")}b.p.footerrow&&(T=T+("<td role='gridcell' "+o(d,0,"",null,"",false)+">&#160;</td>"))}).mousedown(function(d){if(a(d.target).closest("th>span.ui-jqgrid-resize").length===1){var c=Z(this);if(b.p.forceFit===true){var f=b.p,g=c,i;for(i=c+1;i<b.p.colModel.length;i++)if(b.p.colModel[i].hidden!==
true){g=i;break}f.nv=g-c}e.dragStart(c,d,ca(c));return false}}).click(function(d){if(b.p.disableClick)return b.p.disableClick=false;var c="th>div.ui-jqgrid-sortable",e,f;b.p.viewsortcols[2]||(c="th>div>span>span.ui-grid-ico-sort");d=a(d.target).closest(c);if(d.length===1){c=Z(this);if(!b.p.viewsortcols[2]){e=true;f=d.attr("sort")}W(a("div",this)[0].id,c,e,f);return false}});if(b.p.sortable&&a.fn.sortable)try{a(b).jqGrid("sortableColumns",j)}catch(ja){}b.p.footerrow&&(T+="</tr></tbody></table>");ba+=
"</tr>";this.appendChild(document.createElement("tbody"));a(this).addClass("ui-jqgrid-btable").append(ba);var ba=null,j=a("<table class='ui-jqgrid-htable' style='width:"+b.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(C),R=b.p.caption&&!0===b.p.hiddengrid?!0:!1;g=a("<div class='ui-jqgrid-hbox"+("rtl"===i?"-rtl":"")+"'></div>");C=null;e.hDiv=document.createElement("div");a(e.hDiv).css({width:e.width+"px"}).addClass("ui-state-default ui-jqgrid-hdiv").append(g);
a(g).append(j);j=null;R&&a(e.hDiv).hide();b.p.pager&&("string"===typeof b.p.pager?"#"!==b.p.pager.substr(0,1)&&(b.p.pager="#"+b.p.pager):b.p.pager="#"+a(b.p.pager).attr("id"),a(b.p.pager).css({width:e.width+"px"}).addClass("ui-state-default ui-jqgrid-pager ui-corner-bottom").appendTo(m),R&&a(b.p.pager).hide(),V(b.p.pager,""));!1===b.p.cellEdit&&!0===b.p.hoverrows&&a(b).bind("mouseover",function(b){y=a(b.target).closest("tr.jqgrow");a(y).attr("class")!=="ui-subgrid"&&a(y).addClass("ui-state-hover")}).bind("mouseout",
function(b){y=a(b.target).closest("tr.jqgrow");a(y).removeClass("ui-state-hover")});var K,U,fa;a(b).before(e.hDiv).click(function(d){A=d.target;y=a(A,b.rows).closest("tr.jqgrow");if(a(y).length===0||y[0].className.indexOf("ui-state-disabled")>-1||(a(A,b).closest("table.ui-jqgrid-btable").attr("id")||"").replace("_frozen","")!==b.id)return this;var c=a(A).hasClass("cbox"),e=a(b).triggerHandler("jqGridBeforeSelectRow",[y[0].id,d]);(e=e===false||e==="stop"?false:true)&&a.isFunction(b.p.beforeSelectRow)&&
(e=b.p.beforeSelectRow.call(b,y[0].id,d));if(!(A.tagName==="A"||(A.tagName==="INPUT"||A.tagName==="TEXTAREA"||A.tagName==="OPTION"||A.tagName==="SELECT")&&!c)&&e===true){K=y[0].id;U=a.jgrid.getCellIndex(A);fa=a(A).closest("td,th").html();a(b).triggerHandler("jqGridCellSelect",[K,U,fa,d]);a.isFunction(b.p.onCellSelect)&&b.p.onCellSelect.call(b,K,U,fa,d);if(b.p.cellEdit===true)if(b.p.multiselect&&c)a(b).jqGrid("setSelection",K,true,d);else{K=y[0].rowIndex;try{a(b).jqGrid("editCell",K,U,true)}catch(f){}}else if(b.p.multikey)if(d[b.p.multikey])a(b).jqGrid("setSelection",
K,true,d);else{if(b.p.multiselect&&c){c=a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+K).is(":checked");a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+K)[b.p.useProp?"prop":"attr"]("checked",c)}}else{if(b.p.multiselect&&b.p.multiboxonly&&!c){var g=b.p.frozenColumns?b.p.id+"_frozen":"";a(b.p.selarrrow).each(function(d,c){var e=b.rows.namedItem(c);a(e).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+a.jgrid.jqID(c))[b.p.useProp?"prop":"attr"]("checked",false);if(g){a("#"+a.jgrid.jqID(c),"#"+a.jgrid.jqID(g)).removeClass("ui-state-highlight");
a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+a.jgrid.jqID(c),"#"+a.jgrid.jqID(g))[b.p.useProp?"prop":"attr"]("checked",false)}});b.p.selarrrow=[]}a(b).jqGrid("setSelection",K,true,d)}}}).bind("reloadGrid",function(d,c){if(b.p.treeGrid===true)b.p.datatype=b.p.treedatatype;c&&c.current&&b.grid.selectionPreserver(b);if(b.p.datatype==="local"){a(b).jqGrid("resetSelection");b.p.data.length&&z()}else if(!b.p.treeGrid){b.p.selrow=null;if(b.p.multiselect){b.p.selarrrow=[];O(false)}b.p.savedRow=[]}b.p.scroll&&w.call(b,
true,false);if(c&&c.page){var e=c.page;if(e>b.p.lastpage)e=b.p.lastpage;e<1&&(e=1);b.p.page=e;b.grid.bDiv.scrollTop=b.grid.prevRowHeight?(e-1)*b.grid.prevRowHeight*b.p.rowNum:0}if(b.grid.prevRowHeight&&b.p.scroll){delete b.p.lastpage;b.grid.populateVisible()}else b.grid.populate();b.p._inlinenav===true&&a(b).jqGrid("showAddEditButtons");return false}).dblclick(function(d){A=d.target;y=a(A,b.rows).closest("tr.jqgrow");if(a(y).length!==0){K=y[0].rowIndex;U=a.jgrid.getCellIndex(A);a(b).triggerHandler("jqGridDblClickRow",
[a(y).attr("id"),K,U,d]);a.isFunction(b.p.ondblClickRow)&&b.p.ondblClickRow.call(b,a(y).attr("id"),K,U,d)}}).bind("contextmenu",function(d){A=d.target;y=a(A,b.rows).closest("tr.jqgrow");if(a(y).length!==0){b.p.multiselect||a(b).jqGrid("setSelection",y[0].id,true,d);K=y[0].rowIndex;U=a.jgrid.getCellIndex(A);a(b).triggerHandler("jqGridRightClickRow",[a(y).attr("id"),K,U,d]);a.isFunction(b.p.onRightClickRow)&&b.p.onRightClickRow.call(b,a(y).attr("id"),K,U,d)}});e.bDiv=document.createElement("div");k&&
"auto"===(""+b.p.height).toLowerCase()&&(b.p.height="100%");a(e.bDiv).append(a('<div style="position:relative;'+(k&&8>a.jgrid.msiever()?"height:0.01%;":"")+'"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:b.p.height+(isNaN(b.p.height)?"":"px"),width:e.width+"px"}).scroll(e.scrollGrid);a("table:first",e.bDiv).css({width:b.p.tblwidth+"px"});a.support.tbody||2===a("tbody",this).length&&a("tbody:gt(0)",this).remove();b.p.multikey&&(a.jgrid.msie?a(e.bDiv).bind("selectstart",
function(){return false}):a(e.bDiv).bind("mousedown",function(){return false}));R&&a(e.bDiv).hide();e.cDiv=document.createElement("div");var ga=!0===b.p.hidegrid?a("<a role='link' href='javascript:void(0)'/>").addClass("ui-jqgrid-titlebar-close HeaderButton").hover(function(){ga.addClass("ui-state-hover")},function(){ga.removeClass("ui-state-hover")}).append("<span class='ui-icon ui-icon-circle-triangle-n'></span>").css("rtl"===i?"left":"right","0px"):"";a(e.cDiv).append(ga).append("<span class='ui-jqgrid-title"+
("rtl"===i?"-rtl":"")+"'>"+b.p.caption+"</span>").addClass("ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix");a(e.cDiv).insertBefore(e.hDiv);b.p.toolbar[0]&&(e.uDiv=document.createElement("div"),"top"===b.p.toolbar[1]?a(e.uDiv).insertBefore(e.hDiv):"bottom"===b.p.toolbar[1]&&a(e.uDiv).insertAfter(e.hDiv),"both"===b.p.toolbar[1]?(e.ubDiv=document.createElement("div"),a(e.uDiv).addClass("ui-userdata ui-state-default").attr("id","t_"+this.id).insertBefore(e.hDiv),a(e.ubDiv).addClass("ui-userdata ui-state-default").attr("id",
"tb_"+this.id).insertAfter(e.hDiv),R&&a(e.ubDiv).hide()):a(e.uDiv).width(e.width).addClass("ui-userdata ui-state-default").attr("id","t_"+this.id),R&&a(e.uDiv).hide());b.p.toppager&&(b.p.toppager=a.jgrid.jqID(b.p.id)+"_toppager",e.topDiv=a("<div id='"+b.p.toppager+"'></div>")[0],b.p.toppager="#"+b.p.toppager,a(e.topDiv).addClass("ui-state-default ui-jqgrid-toppager").width(e.width).insertBefore(e.hDiv),V(b.p.toppager,"_t"));b.p.footerrow&&(e.sDiv=a("<div class='ui-jqgrid-sdiv'></div>")[0],g=a("<div class='ui-jqgrid-hbox"+
("rtl"===i?"-rtl":"")+"'></div>"),a(e.sDiv).append(g).width(e.width).insertAfter(e.hDiv),a(g).append(T),e.footers=a(".ui-jqgrid-ftable",e.sDiv)[0].rows[0].cells,b.p.rownumbers&&(e.footers[0].className="ui-state-default jqgrid-rownum"),R&&a(e.sDiv).hide());g=null;if(b.p.caption){var ia=b.p.datatype;!0===b.p.hidegrid&&(a(".ui-jqgrid-titlebar-close",e.cDiv).click(function(d){var c=a.isFunction(b.p.onHeaderClick),f=".ui-jqgrid-bdiv, .ui-jqgrid-hdiv, .ui-jqgrid-pager, .ui-jqgrid-sdiv",g,i=this;if(b.p.toolbar[0]===
true){b.p.toolbar[1]==="both"&&(f=f+(", #"+a(e.ubDiv).attr("id")));f=f+(", #"+a(e.uDiv).attr("id"))}g=a(f,"#gview_"+a.jgrid.jqID(b.p.id)).length;b.p.gridstate==="visible"?a(f,"#gbox_"+a.jgrid.jqID(b.p.id)).slideUp("fast",function(){g--;if(g===0){a("span",i).removeClass("ui-icon-circle-triangle-n").addClass("ui-icon-circle-triangle-s");b.p.gridstate="hidden";a("#gbox_"+a.jgrid.jqID(b.p.id)).hasClass("ui-resizable")&&a(".ui-resizable-handle","#gbox_"+a.jgrid.jqID(b.p.id)).hide();a(b).triggerHandler("jqGridHeaderClick",
[b.p.gridstate,d]);c&&(R||b.p.onHeaderClick.call(b,b.p.gridstate,d))}}):b.p.gridstate==="hidden"&&a(f,"#gbox_"+a.jgrid.jqID(b.p.id)).slideDown("fast",function(){g--;if(g===0){a("span",i).removeClass("ui-icon-circle-triangle-s").addClass("ui-icon-circle-triangle-n");if(R){b.p.datatype=ia;D();R=false}b.p.gridstate="visible";a("#gbox_"+a.jgrid.jqID(b.p.id)).hasClass("ui-resizable")&&a(".ui-resizable-handle","#gbox_"+a.jgrid.jqID(b.p.id)).show();a(b).triggerHandler("jqGridHeaderClick",[b.p.gridstate,
d]);c&&(R||b.p.onHeaderClick.call(b,b.p.gridstate,d))}});return false}),R&&(b.p.datatype="local",a(".ui-jqgrid-titlebar-close",e.cDiv).trigger("click")))}else a(e.cDiv).hide();a(e.hDiv).after(e.bDiv).mousemove(function(a){if(e.resizing){e.dragMove(a);return false}});a(".ui-jqgrid-labels",e.hDiv).bind("selectstart",function(){return false});a(document).mouseup(function(){if(e.resizing){e.dragEnd();return false}return true});b.formatCol=o;b.sortData=W;b.updatepager=function(d,c){var e,f,g,i,h,j,l,m=
"",k=b.p.pager?"_"+a.jgrid.jqID(b.p.pager.substr(1)):"",o=b.p.toppager?"_"+b.p.toppager.substr(1):"";g=parseInt(b.p.page,10)-1;g<0&&(g=0);g=g*parseInt(b.p.rowNum,10);h=g+b.p.reccount;if(b.p.scroll){e=a("tbody:first > tr:gt(0)",b.grid.bDiv);g=h-e.length;b.p.reccount=e.length;if(e=e.outerHeight()||b.grid.prevRowHeight){f=g*e;l=parseInt(b.p.records,10)*e;a(">div:first",b.grid.bDiv).css({height:l}).children("div:first").css({height:f,display:f?"":"none"});if(b.grid.bDiv.scrollTop==0&&b.p.page>1)b.grid.bDiv.scrollTop=
b.p.rowNum*(b.p.page-1)*e}b.grid.bDiv.scrollLeft=b.grid.hDiv.scrollLeft}m=b.p.pager||"";if(m=m+(b.p.toppager?m?","+b.p.toppager:b.p.toppager:"")){l=a.jgrid.formatter.integer||{};e=n(b.p.page);f=n(b.p.lastpage);a(".selbox",m)[this.p.useProp?"prop":"attr"]("disabled",false);if(b.p.pginput===true){a(".ui-pg-input",m).val(b.p.page);i=b.p.toppager?"#sp_1"+k+",#sp_1"+o:"#sp_1"+k;a(i).html(a.fmatter?a.fmatter.util.NumberFormat(b.p.lastpage,l):b.p.lastpage)}if(b.p.viewrecords)if(b.p.reccount===0)a(".ui-paging-info",
m).html(b.p.emptyrecords);else{i=g+1;j=b.p.records;if(a.fmatter){i=a.fmatter.util.NumberFormat(i,l);h=a.fmatter.util.NumberFormat(h,l);j=a.fmatter.util.NumberFormat(j,l)}a(".ui-paging-info",m).html(a.jgrid.format(b.p.recordtext,i,h,j))}if(b.p.pgbuttons===true){e<=0&&(e=f=0);if(e===1||e===0){a("#first"+k+", #prev"+k).addClass("ui-state-disabled").removeClass("ui-state-hover");b.p.toppager&&a("#first_t"+o+", #prev_t"+o).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#first"+k+", #prev"+
k).removeClass("ui-state-disabled");b.p.toppager&&a("#first_t"+o+", #prev_t"+o).removeClass("ui-state-disabled")}if(e===f||e===0){a("#next"+k+", #last"+k).addClass("ui-state-disabled").removeClass("ui-state-hover");b.p.toppager&&a("#next_t"+o+", #last_t"+o).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#next"+k+", #last"+k).removeClass("ui-state-disabled");b.p.toppager&&a("#next_t"+o+", #last_t"+o).removeClass("ui-state-disabled")}}}d===true&&b.p.rownumbers===true&&a(">td.jqgrid-rownum",
b.rows).each(function(b){a(this).html(g+1+b)});c&&b.p.jqgdnd&&a(b).jqGrid("gridDnD","updateDnD");a(b).triggerHandler("jqGridGridComplete");a.isFunction(b.p.gridComplete)&&b.p.gridComplete.call(b);a(b).triggerHandler("jqGridAfterGridComplete")};b.refreshIndex=z;b.setHeadCheckBox=O;b.constructTr=J;b.formatter=function(a,b,d,c,e){return p(a,b,d,c,e)};a.extend(e,{populate:D,emptyRows:w});this.grid=e;b.addXmlData=function(a){x(a,b.grid.bDiv)};b.addJSONData=function(a){B(a,b.grid.bDiv)};this.grid.cols=
this.rows[0].cells;a(b).triggerHandler("jqGridInitGrid");a.isFunction(b.p.onInitGrid)&&b.p.onInitGrid.call(b);D();b.p.hiddengrid=!1}}}})};a.jgrid.extend({getGridParam:function(a){var e=this[0];return!e||!e.grid?void 0:!a?e.p:void 0!==e.p[a]?e.p[a]:null},setGridParam:function(c){return this.each(function(){this.grid&&"object"===typeof c&&a.extend(!0,this.p,c)})},getDataIDs:function(){var c=[],e=0,d,f=0;this.each(function(){if((d=this.rows.length)&&0<d)for(;e<d;)a(this.rows[e]).hasClass("jqgrow")&&
(c[f]=this.rows[e].id,f++),e++});return c},setSelection:function(c,e,d){return this.each(function(){var f,b,h,g,i,j;if(void 0!==c&&(e=!1===e?!1:!0,(b=this.rows.namedItem(""+c))&&b.className&&!(-1<b.className.indexOf("ui-state-disabled"))))if(!0===this.p.scrollrows&&(h=this.rows.namedItem(c).rowIndex,0<=h&&(f=a(this.grid.bDiv)[0].clientHeight,g=a(this.grid.bDiv)[0].scrollTop,i=a(this.rows[h]).position().top,h=this.rows[h].clientHeight,i+h>=f+g?a(this.grid.bDiv)[0].scrollTop=i-(f+g)+h+g:i<f+g&&i<g&&
(a(this.grid.bDiv)[0].scrollTop=i))),!0===this.p.frozenColumns&&(j=this.p.id+"_frozen"),this.p.multiselect){if(this.setHeadCheckBox(!1),this.p.selrow=b.id,g=a.inArray(this.p.selrow,this.p.selarrrow),-1===g?("ui-subgrid"!==b.className&&a(b).addClass("ui-state-highlight").attr("aria-selected","true"),f=!0,this.p.selarrrow.push(this.p.selrow)):("ui-subgrid"!==b.className&&a(b).removeClass("ui-state-highlight").attr("aria-selected","false"),f=!1,this.p.selarrrow.splice(g,1),i=this.p.selarrrow[0],this.p.selrow=
...
[b.id,f,d]),this.p.onSelectRow&&this.p.onSelectRow.call(this,b.id,f,d)})},resetSelection:function(c){return this.each(function(){var e=this,d,f,b;!0===e.p.frozenColumns&&(b=e.p.id+"_frozen");if(void 0!==c){f=c===e.p.selrow?e.p.selrow:c;a("#"+a.jgrid.jqID(e.p.id)+" tbody:first tr#"+a.jgrid.jqID(f)).removeClass("ui-state-highlight").attr("aria-selected","false");b&&a("#"+a.jgrid.jqID(f),"#"+a.jgrid.jqID(b)).removeClass("ui-state-highlight");if(e.p.multiselect){a("#jqg_"+a.jgrid.jqID(e.p.id)+"_"+a.jgrid.jqID(f),
"#"+a.jgrid.jqID(e.p.id))[e.p.useProp?"prop":"attr"]("checked",!1);if(b)a("#jqg_"+a.jgrid.jqID(e.p.id)+"_"+a.jgrid.jqID(f),"#"+a.jgrid.jqID(b))[e.p.useProp?"prop":"attr"]("checked",!1);e.setHeadCheckBox(!1)}f=null}else e.p.multiselect?(a(e.p.selarrrow).each(function(c,f){d=e.rows.namedItem(f);a(d).removeClass("ui-state-highlight").attr("aria-selected","false");a("#jqg_"+a.jgrid.jqID(e.p.id)+"_"+a.jgrid.jqID(f))[e.p.useProp?"prop":"attr"]("checked",!1);b&&(a("#"+a.jgrid.jqID(f),"#"+a.jgrid.jqID(b)).removeClass("ui-state-highlight"),
a("#jqg_"+a.jgrid.jqID(e.p.id)+"_"+a.jgrid.jqID(f),"#"+a.jgrid.jqID(b))[e.p.useProp?"prop":"attr"]("checked",!1))}),e.setHeadCheckBox(!1),e.p.selarrrow=[]):e.p.selrow&&(a("#"+a.jgrid.jqID(e.p.id)+" tbody:first tr#"+a.jgrid.jqID(e.p.selrow)).removeClass("ui-state-highlight").attr("aria-selected","false"),b&&a("#"+a.jgrid.jqID(e.p.selrow),"#"+a.jgrid.jqID(b)).removeClass("ui-state-highlight"),e.p.selrow=null);!0===e.p.cellEdit&&0<=parseInt(e.p.iCol,10)&&0<=parseInt(e.p.iRow,10)&&(a("td:eq("+e.p.iCol+
...
a.jgrid.htmlDecode(a("span:first",this).html());else try{e[i]=a.unformat.call(g,this,{rowId:j.id,colModel:g.p.colModel[b]},b)}catch(d){e[i]=a.jgrid.htmlDecode(a(this).html())}}),f&&(d.push(e),e={})),h++});return d||e},delRowData:function(c){var e=!1,d,f;this.each(function(){d=this.rows.namedItem(c);if(!d)return!1;a(d).remove();this.p.records--;this.p.reccount--;this.updatepager(!0,!1);e=!0;this.p.multiselect&&(f=a.inArray(c,this.p.selarrrow),-1!==f&&this.p.selarrrow.splice(f,1));this.p.selrow=this.p.multiselect&&
0<this.p.selarrrow.length?this.p.selarrrow[this.p.selarrrow.length-1]:null;if("local"===this.p.datatype){var b=this.p._index[a.jgrid.stripPref(this.p.idPrefix,c)];void 0!==b&&(this.p.data.splice(b,1),this.refreshIndex())}if(!0===this.p.altRows&&e){var h=this.p.altclass;a(this.rows).each(function(b){b%2===1?a(this).addClass(h):a(this).removeClass(h)})}});return e},setRowData:function(c,e,d){var f,b=!0,h;this.each(function(){if(!this.grid)return!1;var g=this,i,j,k=typeof d,m={};j=g.rows.namedItem(c);
if(!j)return!1;if(e)try{if(a(this.p.colModel).each(function(b){f=this.name;var d=a.jgrid.getAccessor(e,f);void 0!==d&&(m[f]=this.formatter&&"string"===typeof this.formatter&&"date"===this.formatter?a.unformat.date.call(g,d,this):d,i=g.formatter(c,d,b,e,"edit"),h=this.title?{title:a.jgrid.stripHtml(i)}:{},!0===g.p.treeGrid&&f===g.p.ExpandColumn?a("td[role='gridcell']:eq("+b+") > span:first",j).html(i).attr(h):a("td[role='gridcell']:eq("+b+")",j).html(i).attr(h))}),"local"===g.p.datatype){var n=a.jgrid.stripPref(g.p.idPrefix,
c),o=g.p._index[n],l;if(g.p.treeGrid)for(l in g.p.treeReader)g.p.treeReader.hasOwnProperty(l)&&delete m[g.p.treeReader[l]];void 0!==o&&(g.p.data[o]=a.extend(!0,g.p.data[o],m));m=null}}catch(p){b=!1}b&&("string"===k?a(j).addClass(d):"object"===k&&a(j).css(d),a(g).triggerHandler("jqGridAfterGridComplete"))});return b},addRowData:function(c,e,d,f){d||(d="last");var b=!1,h,g,i,j,k,m,n,o,l="",p,q,r,s,t,v;e&&(a.isArray(e)?(p=!0,d="last",q=c):(e=[e],p=!1),this.each(function(){var w=e.length;k=this.p.rownumbers===
true?1:0;i=this.p.multiselect===true?1:0;j=this.p.subGrid===true?1:0;if(!p)if(c!==void 0)c=""+c;else{c=a.jgrid.randId();if(this.p.keyIndex!==false){q=this.p.colModel[this.p.keyIndex+i+j+k].name;e[0][q]!==void 0&&(c=e[0][q])}}r=this.p.altclass;for(var z=0,J="",x={},B=a.isFunction(this.p.afterInsertRow)?true:false;z<w;){s=e[z];g=[];if(p){try{c=s[q];c===void 0&&(c=a.jgrid.randId())}catch(F){c=a.jgrid.randId()}J=this.p.altRows===true?(this.rows.length-1)%2===0?r:"":""}v=c;c=this.p.idPrefix+c;if(k){l=
this.formatCol(0,1,"",null,c,true);g[g.length]='<td role="gridcell" class="ui-state-default jqgrid-rownum" '+l+">0</td>"}if(i){o='<input role="checkbox" type="checkbox" id="jqg_'+this.p.id+"_"+c+'" class="cbox"/>';l=this.formatCol(k,1,"",null,c,true);g[g.length]='<td role="gridcell" '+l+">"+o+"</td>"}j&&(g[g.length]=a(this).jqGrid("addSubGridCell",i+k,1));for(n=i+j+k;n<this.p.colModel.length;n++){t=this.p.colModel[n];h=t.name;x[h]=s[h];o=this.formatter(c,a.jgrid.getAccessor(s,h),n,s);l=this.formatCol(n,
1,o,s,c,x);g[g.length]='<td role="gridcell" '+l+">"+o+"</td>"}g.unshift(this.constructTr(c,false,J,x,s,false));g[g.length]="</tr>";if(this.rows.length===0)a("table:first",this.grid.bDiv).append(g.join(""));else switch(d){case "last":a(this.rows[this.rows.length-1]).after(g.join(""));m=this.rows.length-1;break;case "first":a(this.rows[0]).after(g.join(""));m=1;break;case "after":(m=this.rows.namedItem(f))&&(a(this.rows[m.rowIndex+1]).hasClass("ui-subgrid")?a(this.rows[m.rowIndex+1]).after(g):a(m).after(g.join("")));
m++;break;case "before":if(m=this.rows.namedItem(f)){a(m).before(g.join(""));m=m.rowIndex}m--}this.p.subGrid===true&&a(this).jqGrid("addSubGrid",i+k,m);this.p.records++;this.p.reccount++;a(this).triggerHandler("jqGridAfterInsertRow",[c,s,s]);B&&this.p.afterInsertRow.call(this,c,s,s);z++;if(this.p.datatype==="local"){x[this.p.localReader.id]=v;this.p._index[v]=this.p.data.length;this.p.data.push(x);x={}}}this.p.altRows===true&&!p&&(d==="last"?(this.rows.length-1)%2===1&&a(this.rows[this.rows.length-
1]).addClass(r):a(this.rows).each(function(b){b%2===1?a(this).addClass(r):a(this).removeClass(r)}));this.updatepager(true,true);b=true}));return b},footerData:function(c,e,d){function f(a){for(var b in a)if(a.hasOwnProperty(b))return!1;return!0}var b,h=!1,g={},i;void 0===c&&(c="get");"boolean"!==typeof d&&(d=!0);c=c.toLowerCase();this.each(function(){var j=this,k;if(!j.grid||!j.p.footerrow||"set"===c&&f(e))return!1;h=!0;a(this.p.colModel).each(function(f){b=this.name;"set"===c?void 0!==e[b]&&(k=d?
j.formatter("",e[b],f,e,"edit"):e[b],i=this.title?{title:a.jgrid.stripHtml(k)}:{},a("tr.footrow td:eq("+f+")",j.grid.sDiv).html(k).attr(i),h=!0):"get"===c&&(g[b]=a("tr.footrow td:eq("+f+")",j.grid.sDiv).html())})});return"get"===c?g:h},showHideCol:function(c,e){return this.each(function(){var d=this,f=!1,b=a.jgrid.cell_width?0:d.p.cellLayout,h;if(d.grid){"string"===typeof c&&(c=[c]);e="none"!==e?"":"none";var g=""===e?!0:!1,i=d.p.groupHeader&&("object"===typeof d.p.groupHeader||a.isFunction(d.p.groupHeader));
i&&a(d).jqGrid("destroyGroupHeader",!1);a(this.p.colModel).each(function(i){if(-1!==a.inArray(this.name,c)&&this.hidden===g){if(!0===d.p.frozenColumns&&!0===this.frozen)return!0;a("tr",d.grid.hDiv).each(function(){a(this.cells[i]).css("display",e)});a(d.rows).each(function(){a(this).hasClass("jqgroup")||a(this.cells[i]).css("display",e)});d.p.footerrow&&a("tr.footrow td:eq("+i+")",d.grid.sDiv).css("display",e);h=parseInt(this.width,10);d.p.tblwidth="none"===e?d.p.tblwidth-(h+b):d.p.tblwidth+(h+b);
this.hidden=!g;f=!0;a(d).triggerHandler("jqGridShowHideCol",[g,this.name,i])}});!0===f&&(!0===d.p.shrinkToFit&&!isNaN(d.p.height)&&(d.p.tblwidth+=parseInt(d.p.scrollOffset,10)),a(d).jqGrid("setGridWidth",!0===d.p.shrinkToFit?d.p.tblwidth:d.p.width));i&&a(d).jqGrid("setGroupHeaders",d.p.groupHeader)}})},hideCol:function(c){return this.each(function(){a(this).jqGrid("showHideCol",c,"none")})},showCol:function(c){return this.each(function(){a(this).jqGrid("showHideCol",c,"")})},remapColumns:function(c,
e,d){function f(b){var d;d=b.length?a.makeArray(b):a.extend({},b);a.each(c,function(a){b[a]=d[this]})}function b(b,d){a(">tr"+(d||""),b).each(function(){var b=this,d=a.makeArray(b.cells);a.each(c,function(){var a=d[this];a&&b.appendChild(a)})})}var h=this.get(0);f(h.p.colModel);f(h.p.colNames);f(h.grid.headers);b(a("thead:first",h.grid.hDiv),d&&":not(.ui-jqgrid-labels)");e&&b(a("#"+a.jgrid.jqID(h.p.id)+" tbody:first"),".jqgfirstrow, tr.jqgrow, tr.jqfoot");h.p.footerrow&&b(a("tbody:first",h.grid.sDiv));
h.p.remapColumns&&(h.p.remapColumns.length?f(h.p.remapColumns):h.p.remapColumns=a.makeArray(c));h.p.lastsort=a.inArray(h.p.lastsort,c);h.p.treeGrid&&(h.p.expColInd=a.inArray(h.p.expColInd,c));a(h).triggerHandler("jqGridRemapColumns",[c,e,d])},setGridWidth:function(c,e){return this.each(function(){if(this.grid){var d=this,f,b=0,h=a.jgrid.cell_width?0:d.p.cellLayout,g,i=0,j=!1,k=d.p.scrollOffset,m,n=0,o=0,l;"boolean"!==typeof e&&(e=d.p.shrinkToFit);if(!isNaN(c)){c=parseInt(c,10);d.grid.width=d.p.width=
c;a("#gbox_"+a.jgrid.jqID(d.p.id)).css("width",c+"px");a("#gview_"+a.jgrid.jqID(d.p.id)).css("width",c+"px");a(d.grid.bDiv).css("width",c+"px");a(d.grid.hDiv).css("width",c+"px");d.p.pager&&a(d.p.pager).css("width",c+"px");d.p.toppager&&a(d.p.toppager).css("width",c+"px");!0===d.p.toolbar[0]&&(a(d.grid.uDiv).css("width",c+"px"),"both"===d.p.toolbar[1]&&a(d.grid.ubDiv).css("width",c+"px"));d.p.footerrow&&a(d.grid.sDiv).css("width",c+"px");!1===e&&!0===d.p.forceFit&&(d.p.forceFit=!1);if(!0===e){a.each(d.p.colModel,
function(){if(this.hidden===false){f=this.widthOrg;b=b+(f+h);this.fixed?n=n+(f+h):i++;o++}});if(0===i)return;d.p.tblwidth=b;m=c-h*i-n;if(!isNaN(d.p.height)&&(a(d.grid.bDiv)[0].clientHeight<a(d.grid.bDiv)[0].scrollHeight||1===d.rows.length))j=!0,m-=k;var b=0,p=0<d.grid.cols.length;a.each(d.p.colModel,function(a){if(this.hidden===false&&!this.fixed){f=this.widthOrg;f=Math.round(m*f/(d.p.tblwidth-h*i-n));if(!(f<0)){this.width=f;b=b+f;d.grid.headers[a].width=f;d.grid.headers[a].el.style.width=f+"px";
if(d.p.footerrow)d.grid.footers[a].style.width=f+"px";if(p)d.grid.cols[a].style.width=f+"px";g=a}}});if(!g)return;l=0;j?c-n-(b+h*i)!==k&&(l=c-n-(b+h*i)-k):1!==Math.abs(c-n-(b+h*i))&&(l=c-n-(b+h*i));d.p.colModel[g].width+=l;d.p.tblwidth=b+l+h*i+n;d.p.tblwidth>c?(j=d.p.tblwidth-parseInt(c,10),d.p.tblwidth=c,f=d.p.colModel[g].width-=j):f=d.p.colModel[g].width;d.grid.headers[g].width=f;d.grid.headers[g].el.style.width=f+"px";p&&(d.grid.cols[g].style.width=f+"px");d.p.footerrow&&(d.grid.footers[g].style.width=
...
c;this.p.scroll&&this.grid.populateVisible()}})},setCaption:function(c){return this.each(function(){this.p.caption=c;a("span.ui-jqgrid-title, span.ui-jqgrid-title-rtl",this.grid.cDiv).html(c);a(this.grid.cDiv).show()})},setLabel:function(c,e,d,f){return this.each(function(){var b=-1;if(this.grid&&void 0!==c&&(a(this.p.colModel).each(function(a){if(this.name===c)return b=a,!1}),0<=b)){var h=a("tr.ui-jqgrid-labels th:eq("+b+")",this.grid.hDiv);if(e){var g=a(".s-ico",h);a("[id^=jqgh_]",h).empty().html(e).append(g);
this.p.colNames[b]=e}d&&("string"===typeof d?a(h).addClass(d):a(h).css(d));"object"===typeof f&&a(h).attr(f)}})},setCell:function(c,e,d,f,b,h){return this.each(function(){var g=-1,i,j;if(this.grid&&(isNaN(e)?a(this.p.colModel).each(function(a){if(this.name===e)return g=a,!1}):g=parseInt(e,10),0<=g&&(i=this.rows.namedItem(c)))){var k=a("td:eq("+g+")",i);if(""!==d||!0===h)i=this.formatter(c,d,g,i,"edit"),j=this.p.colModel[g].title?{title:a.jgrid.stripHtml(i)}:{},this.p.treeGrid&&0<a(".tree-wrap",a(k)).length?
a("span",a(k)).html(i).attr(j):a(k).html(i).attr(j),"local"===this.p.datatype&&(i=this.p.colModel[g],d=i.formatter&&"string"===typeof i.formatter&&"date"===i.formatter?a.unformat.date.call(this,d,i):d,j=this.p._index[a.jgrid.stripPref(this.p.idPrefix,c)],void 0!==j&&(this.p.data[j][i.name]=d));"string"===typeof f?a(k).addClass(f):f&&a(k).css(f);"object"===typeof b&&a(k).attr(b)}})},getCell:function(c,e){var d=!1;this.each(function(){var f=-1;if(this.grid&&(isNaN(e)?a(this.p.colModel).each(function(a){if(this.name===
e)return f=a,!1}):f=parseInt(e,10),0<=f)){var b=this.rows.namedItem(c);if(b)try{d=a.unformat.call(this,a("td:eq("+f+")",b),{rowId:b.id,colModel:this.p.colModel[f]},f)}catch(h){d=a.jgrid.htmlDecode(a("td:eq("+f+")",b).html())}}});return d},getCol:function(c,e,d){var f=[],b,h=0,g,i,j,e="boolean"!==typeof e?!1:e;void 0===d&&(d=!1);this.each(function(){var k=-1;if(this.grid&&(isNaN(c)?a(this.p.colModel).each(function(a){if(this.name===c)return k=a,!1}):k=parseInt(c,10),0<=k)){var m=this.rows.length,n=
0;if(m&&0<m){for(;n<m;){if(a(this.rows[n]).hasClass("jqgrow")){try{b=a.unformat.call(this,a(this.rows[n].cells[k]),{rowId:this.rows[n].id,colModel:this.p.colModel[k]},k)}catch(o){b=a.jgrid.htmlDecode(this.rows[n].cells[k].innerHTML)}d?(j=parseFloat(b),h+=j,void 0===i&&(i=g=j),g=Math.min(g,j),i=Math.max(i,j)):e?f.push({id:this.rows[n].id,value:b}):f.push(b)}n++}if(d)switch(d.toLowerCase()){case "sum":f=h;break;case "avg":f=h/m;break;case "count":f=m;break;case "min":f=g;break;case "max":f=i}}}});return f},
clearGridData:function(c){return this.each(function(){if(this.grid){"boolean"!==typeof c&&(c=!1);if(this.p.deepempty)a("#"+a.jgrid.jqID(this.p.id)+" tbody:first tr:gt(0)").remove();else{var e=a("#"+a.jgrid.jqID(this.p.id)+" tbody:first tr:first")[0];a("#"+a.jgrid.jqID(this.p.id)+" tbody:first").empty().append(e)}this.p.footerrow&&c&&a(".ui-jqgrid-ftable td",this.grid.sDiv).html("&#160;");this.p.selrow=null;this.p.selarrrow=[];this.p.savedRow=[];this.p.records=0;this.p.page=1;this.p.lastpage=0;this.p.reccount=
0;this.p.data=[];this.p._index={};this.updatepager(!0,!1)}})},getInd:function(a,e){var d=!1,f;this.each(function(){(f=this.rows.namedItem(a))&&(d=!0===e?f:f.rowIndex)});return d},bindKeys:function(c){var e=a.extend({onEnter:null,onSpace:null,onLeftKey:null,onRightKey:null,scrollingRows:!0},c||{});return this.each(function(){var d=this;a("body").is("[role]")||a("body").attr("role","application");d.p.scrollrows=e.scrollingRows;a(d).keydown(function(c){var b=a(d).find("tr[tabindex=0]")[0],h,g,i,j=d.p.treeReader.expanded_field;
if(b)if(i=d.p._index[a.jgrid.stripPref(d.p.idPrefix,b.id)],37===c.keyCode||38===c.keyCode||39===c.keyCode||40===c.keyCode){if(38===c.keyCode){g=b.previousSibling;h="";if(g)if(a(g).is(":hidden"))for(;g;){if(g=g.previousSibling,!a(g).is(":hidden")&&a(g).hasClass("jqgrow")){h=g.id;break}}else h=g.id;a(d).jqGrid("setSelection",h,!0,c);c.preventDefault()}if(40===c.keyCode){g=b.nextSibling;h="";if(g)if(a(g).is(":hidden"))for(;g;){if(g=g.nextSibling,!a(g).is(":hidden")&&a(g).hasClass("jqgrow")){h=g.id;break}}else h=
g.id;a(d).jqGrid("setSelection",h,!0,c);c.preventDefault()}37===c.keyCode&&(d.p.treeGrid&&d.p.data[i][j]&&a(b).find("div.treeclick").trigger("click"),a(d).triggerHandler("jqGridKeyLeft",[d.p.selrow]),a.isFunction(e.onLeftKey)&&e.onLeftKey.call(d,d.p.selrow));39===c.keyCode&&(d.p.treeGrid&&!d.p.data[i][j]&&a(b).find("div.treeclick").trigger("click"),a(d).triggerHandler("jqGridKeyRight",[d.p.selrow]),a.isFunction(e.onRightKey)&&e.onRightKey.call(d,d.p.selrow))}else 13===c.keyCode?(a(d).triggerHandler("jqGridKeyEnter",
[d.p.selrow]),a.isFunction(e.onEnter)&&e.onEnter.call(d,d.p.selrow)):32===c.keyCode&&(a(d).triggerHandler("jqGridKeySpace",[d.p.selrow]),a.isFunction(e.onSpace)&&e.onSpace.call(d,d.p.selrow))})})},unbindKeys:function(){return this.each(function(){a(this).unbind("keydown")})},getLocalRow:function(c){var e=!1,d;this.each(function(){void 0!==c&&(d=this.p._index[a.jgrid.stripPref(this.p.idPrefix,c)],0<=d&&(e=this.p.data[d]))});return e}})})(jQuery);
...
b){0<p&&(l+=",");l+='{"field":"'+a+'",';l+='"op":"'+m[a]+'",';l+='"data":"'+(b+"").replace(/\\/g,"\\\\").replace(/\"/g,'\\"')+'"}';p++});l+="]}";a.extend(e.p.postData,{filters:l});a.each(["searchField","searchString","searchOper"],function(a,b){e.p.postData.hasOwnProperty(b)&&delete e.p.postData[b]})}else a.extend(e.p.postData,b);var q;e.p.searchurl&&(q=e.p.url,a(e).jqGrid("setGridParam",{url:e.p.searchurl}));var r="stop"===a(e).triggerHandler("jqGridToolbarBeforeSearch")?!0:!1;!r&&a.isFunction(c.beforeSearch)&&
(r=c.beforeSearch.call(e));r||a(e).jqGrid("setGridParam",{search:o}).trigger("reloadGrid",[{page:1}]);q&&a(e).jqGrid("setGridParam",{url:q});a(e).triggerHandler("jqGridToolbarAfterSearch");a.isFunction(c.afterSearch)&&c.afterSearch.call(e)},f=function(b,f,h){a("#sopt_menu").remove();var f=parseInt(f,10),h=parseInt(h,10)+18,f='<ul id="sopt_menu" class="ui-search-menu" role="menu" tabindex="0" style="font-size:'+(a(".ui-jqgrid-view").css("font-size")||"11px")+";left:"+f+"px;top:"+h+'px;">',k=e.p.colModel[parseInt(a(b).attr("colindex"))],
h=a(b).attr("soper"),m=[],n,o,l=a.extend({},k.searchoptions);l.sopt||(l.sopt=[],l.sopt[0]="select"===k.stype?"eq":c.defaultSearch);a.each(c.odata,function(){m.push(this.oper)});for(n=0;n<l.sopt.length;n++)o=a.inArray(l.sopt[n],m),-1!==o&&(k=h===c.odata[o].oper?"ui-state-highlight":"",f+='<li class="ui-menu-item '+k+'" role="presentation"><a href="#" class="ui-corner-all g-menu-item" tabindex="0" role="menuitem" value="'+c.odata[o].oper+'" oper="'+c.operands[c.odata[o].oper]+'"><table cellspacing="0" cellpadding="0" border="0"><tr><td width="25px">'+
c.operands[c.odata[o].oper]+"</td><td>"+c.odata[o].text+"</td></tr></table></a></li>");a("body").append(f+"</ul>");a("#sopt_menu").addClass("ui-menu ui-widget ui-widget-content ui-corner-all");a("#sopt_menu > li > a").hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")}).click(function(){var f=a(this).attr("value"),i=a(this).attr("oper");a(e).triggerHandler("jqGridToolbarSelectOper",[f,i,b]);a("#sopt_menu").hide();a(b).text(i).attr("soper",f);if(c.autosearch===
true){i=a(b).parent().next().children()[0];(a(i).val()||f==="nu"||f==="nn")&&d()}})},b=a("<tr class='ui-search-toolbar' role='rowheader'></tr>"),h;a.each(e.p.colModel,function(f){var i=this,j,k;k="";var m="=",n=a("<th role='columnheader' class='ui-state-default ui-th-column ui-th-"+e.p.direction+"'></th>"),o=a("<div style='position:relative;height:100%;padding-right:0.3em;padding-left:0.3em;'></div>"),l=a("<table class='ui-search-table' cellspacing='0'><tr><td class='ui-search-oper'></td><td class='ui-search-input'></td></tr></table>");
!0===this.hidden&&a(n).css("display","none");this.search=!1===this.search?!1:!0;void 0===this.stype&&(this.stype="text");j=a.extend({},this.searchoptions||{});if(this.search){if(c.searchOperators){k=j.sopt?j.sopt[0]:"select"===i.stype?"eq":c.defaultSearch;for(var p=0;p<c.odata.length;p++)if(c.odata[p].oper===k){m=c.operands[k]||"";break}k="<a href='#' title='"+(null!=j.searchtitle?j.searchtitle:c.operandTitle)+"' style='padding-right: 0.5em;' soper='"+k+"' class='soptclass' colindex='"+f+"'>"+m+"</a>"}a("td:eq(0)",
l).append(k);switch(this.stype){case "select":if(k=this.surl||j.dataUrl)a.ajax(a.extend({url:k,dataType:"html",success:function(b){if(j.buildSelect!==void 0){if(b=j.buildSelect(b)){a("td:eq(1)",l).append(b);a(o).append(l)}}else{a("td:eq(1)",l).append(b);a(o).append(l)}j.defaultValue!==void 0&&a("select",o).val(j.defaultValue);a("select",o).attr({name:i.index||i.name,id:"gs_"+i.name});j.attr&&a("select",o).attr(j.attr);a("select",o).css({width:"100%"});a.jgrid.bindEv.call(e,a("select",o)[0],j);c.autosearch===
true&&a("select",o).change(function(){d();return false});b=null}},a.jgrid.ajaxOptions,e.p.ajaxSelectOptions||{}));else{var q,r,s;i.searchoptions?(q=void 0===i.searchoptions.value?"":i.searchoptions.value,r=void 0===i.searchoptions.separator?":":i.searchoptions.separator,s=void 0===i.searchoptions.delimiter?";":i.searchoptions.delimiter):i.editoptions&&(q=void 0===i.editoptions.value?"":i.editoptions.value,r=void 0===i.editoptions.separator?":":i.editoptions.separator,s=void 0===i.editoptions.delimiter?
";":i.editoptions.delimiter);if(q){f=document.createElement("select");f.style.width="100%";a(f).attr({name:i.index||i.name,id:"gs_"+i.name});var t;if("string"===typeof q){k=q.split(s);for(t=0;t<k.length;t++)q=k[t].split(r),s=document.createElement("option"),s.value=q[0],s.innerHTML=q[1],f.appendChild(s)}else if("object"===typeof q)for(t in q)q.hasOwnProperty(t)&&(s=document.createElement("option"),s.value=t,s.innerHTML=q[t],f.appendChild(s));void 0!==j.defaultValue&&a(f).val(j.defaultValue);j.attr&&
a(f).attr(j.attr);a.jgrid.bindEv.call(e,f,j);a("td:eq(1)",l).append(f);a(o).append(l);!0===c.autosearch&&a(f).change(function(){d();return false})}}break;case "text":r=void 0!==j.defaultValue?j.defaultValue:"";a("td:eq(1)",l).append("<input type='text' style='width:100%;padding:0px;' name='"+(i.index||i.name)+"' id='gs_"+i.name+"' value='"+r+"'/>");a(o).append(l);j.attr&&a("input",o).attr(j.attr);a.jgrid.bindEv.call(e,a("input",o)[0],j);!0===c.autosearch&&(c.searchOnEnter?a("input",o).keypress(function(a){if((a.charCode||
a.keyCode||0)===13){d();return false}return this}):a("input",o).keydown(function(a){switch(a.which){case 13:return false;case 9:case 16:case 37:case 38:case 39:case 40:case 27:break;default:h&&clearTimeout(h);h=setTimeout(function(){d()},500)}}));break;case "custom":a("td:eq(1)",l).append("<span style='width:95%;padding:0px;' name='"+(i.index||i.name)+"' id='gs_"+i.name+"'/>");a(o).append(l);try{if(a.isFunction(j.custom_element)){var v=j.custom_element.call(e,void 0!==j.defaultValue?j.defaultValue:
"",j);if(v)v=a(v).addClass("customelement"),a(o).find(">span").append(v);else throw"e2";}else throw"e1";}catch(w){"e1"===w&&a.jgrid.info_dialog(a.jgrid.errors.errcap,"function 'custom_element' "+a.jgrid.edit.msg.nodefined,a.jgrid.edit.bClose),"e2"===w?a.jgrid.info_dialog(a.jgrid.errors.errcap,"function 'custom_element' "+a.jgrid.edit.msg.novalue,a.jgrid.edit.bClose):a.jgrid.info_dialog(a.jgrid.errors.errcap,"string"===typeof w?w:w.message,a.jgrid.edit.bClose)}}}a(n).append(o);a(b).append(n);c.searchOperators||
a("td:eq(0)",l).hide()});a("table thead",e.grid.hDiv).append(b);c.searchOperators&&a(".soptclass").click(function(){var b=a(this).offset();f(this,b.left,b.top)});this.ftoolbar=!0;this.triggerToolbar=d;this.clearToolbar=function(b){var d={},f=0,h,b="boolean"!==typeof b?!0:b;a.each(e.p.colModel,function(){var b,c=a("#gs_"+a.jgrid.jqID(this.name),!0===this.frozen&&!0===e.p.frozenColumns?e.grid.fhDiv:e.grid.hDiv);this.searchoptions&&void 0!==this.searchoptions.defaultValue&&(b=this.searchoptions.defaultValue);
h=this.index||this.name;switch(this.stype){case "select":c.find("option").each(function(d){if(d===0)this.selected=true;if(a(this).val()===b){this.selected=true;return false}});if(void 0!==b)d[h]=b,f++;else try{delete e.p.postData[h]}catch(g){}break;case "text":c.val(b);if(void 0!==b)d[h]=b,f++;else try{delete e.p.postData[h]}catch(l){}break;case "custom":a.isFunction(this.searchoptions.custom_value)&&0<c.length&&"SPAN"===c[0].nodeName.toUpperCase()&&this.searchoptions.custom_value.call(e,c.children(".customelement:first"),
"set",b)}});var m=0<f?!0:!1;if(!0===c.stringResult||"local"===e.p.datatype){var n='{"groupOp":"'+c.groupOp+'","rules":[',o=0;a.each(d,function(a,b){0<o&&(n+=",");n+='{"field":"'+a+'",';n+='"op":"eq",';n+='"data":"'+(b+"").replace(/\\/g,"\\\\").replace(/\"/g,'\\"')+'"}';o++});n+="]}";a.extend(e.p.postData,{filters:n});a.each(["searchField","searchString","searchOper"],function(a,b){e.p.postData.hasOwnProperty(b)&&delete e.p.postData[b]})}else a.extend(e.p.postData,d);var l;e.p.searchurl&&(l=e.p.url,
a(e).jqGrid("setGridParam",{url:e.p.searchurl}));var p="stop"===a(e).triggerHandler("jqGridToolbarBeforeClear")?!0:!1;!p&&a.isFunction(c.beforeClear)&&(p=c.beforeClear.call(e));p||b&&a(e).jqGrid("setGridParam",{search:m}).trigger("reloadGrid",[{page:1}]);l&&a(e).jqGrid("setGridParam",{url:l});a(e).triggerHandler("jqGridToolbarAfterClear");a.isFunction(c.afterClear)&&c.afterClear()};this.toggleToolbar=function(){var b=a("tr.ui-search-toolbar",e.grid.hDiv),d=!0===e.p.frozenColumns?a("tr.ui-search-toolbar",
e.grid.fhDiv):!1;"none"===b.css("display")?(b.show(),d&&d.show()):(b.hide(),d&&d.hide())}}})},destroyFilterToolbar:function(){return this.each(function(){this.ftoolbar&&(this.toggleToolbar=this.clearToolbar=this.triggerToolbar=null,this.ftoolbar=!1,a(this.grid.hDiv).find("table thead tr.ui-search-toolbar").remove())})},destroyGroupHeader:function(c){void 0===c&&(c=!0);return this.each(function(){var e,d,f,b,h,g;d=this.grid;var i=a("table.ui-jqgrid-htable thead",d.hDiv),j=this.p.colModel;if(d){a(this).unbind(".setGroupHeaders");
e=a("<tr>",{role:"rowheader"}).addClass("ui-jqgrid-labels");b=d.headers;d=0;for(f=b.length;d<f;d++){h=j[d].hidden?"none":"";h=a(b[d].el).width(b[d].width).css("display",h);try{h.removeAttr("rowSpan")}catch(k){h.attr("rowSpan",1)}e.append(h);g=h.children("span.ui-jqgrid-resize");0<g.length&&(g[0].style.height="");h.children("div")[0].style.top=""}a(i).children("tr.ui-jqgrid-labels").remove();a(i).prepend(e);!0===c&&a(this).jqGrid("setGridParam",{groupHeader:null})}})},setGroupHeaders:function(c){c=
a.extend({useColSpanStyle:!1,groupHeaders:[]},c||{});return this.each(function(){this.p.groupHeader=c;var e,d,f=0,b,h,g,i,j,k=this.p.colModel,m=k.length,n=this.grid.headers,o=a("table.ui-jqgrid-htable",this.grid.hDiv),l=o.children("thead").children("tr.ui-jqgrid-labels:last").addClass("jqg-second-row-header");b=o.children("thead");var p=o.find(".jqg-first-row-header");void 0===p[0]?p=a("<tr>",{role:"row","aria-hidden":"true"}).addClass("jqg-first-row-header").css("height","auto"):p.empty();var q,
r=function(a,b){var d=b.length,c;for(c=0;c<d;c++)if(b[c].startColumnName===a)return c;return-1};a(this).prepend(b);b=a("<tr>",{role:"rowheader"}).addClass("ui-jqgrid-labels jqg-third-row-header");for(e=0;e<m;e++)if(g=n[e].el,i=a(g),d=k[e],h={height:"0px",width:n[e].width+"px",display:d.hidden?"none":""},a("<th>",{role:"gridcell"}).css(h).addClass("ui-first-th-"+this.p.direction).appendTo(p),g.style.width="",h=r(d.name,c.groupHeaders),0<=h){h=c.groupHeaders[h];f=h.numberOfColumns;j=h.titleText;for(h=
d=0;h<f&&e+h<m;h++)k[e+h].hidden||d++;h=a("<th>").attr({role:"columnheader"}).addClass("ui-state-default ui-th-column-header ui-th-"+this.p.direction).css({height:"22px","border-top":"0px none"}).html(j);0<d&&h.attr("colspan",""+d);this.p.headertitles&&h.attr("title",h.text());0===d&&h.hide();i.before(h);b.append(g);f-=1}else 0===f?c.useColSpanStyle?i.attr("rowspan","2"):(a("<th>",{role:"columnheader"}).addClass("ui-state-default ui-th-column-header ui-th-"+this.p.direction).css({display:d.hidden?
"none":"","border-top":"0px none"}).insertBefore(i),b.append(g)):(b.append(g),f--);k=a(this).children("thead");k.prepend(p);b.insertAfter(l);o.append(k);c.useColSpanStyle&&(o.find("span.ui-jqgrid-resize").each(function(){var b=a(this).parent();b.is(":visible")&&(this.style.cssText="height: "+b.height()+"px !important; cursor: col-resize;")}),o.find("div.ui-jqgrid-sortable").each(function(){var b=a(this),d=b.parent();d.is(":visible")&&d.is(":has(span.ui-jqgrid-resize)")&&b.css("top",(d.height()-b.outerHeight())/
2+"px")}));q=k.find("tr.jqg-first-row-header");a(this).bind("jqGridResizeStop.setGroupHeaders",function(a,b,d){q.find("th").eq(d).width(b)})})},setFrozenColumns:function(){return this.each(function(){if(this.grid){var c=this,e=c.p.colModel,d=0,f=e.length,b=-1,h=!1;if(!(!0===c.p.subGrid||!0===c.p.treeGrid||!0===c.p.cellEdit||c.p.sortable||c.p.scroll||c.p.grouping)){c.p.rownumbers&&d++;for(c.p.multiselect&&d++;d<f;){if(!0===e[d].frozen)h=!0,b=d;else break;d++}if(0<=b&&h){e=c.p.caption?a(c.grid.cDiv).outerHeight():
0;d=a(".ui-jqgrid-htable","#gview_"+a.jgrid.jqID(c.p.id)).height();c.p.toppager&&(e+=a(c.grid.topDiv).outerHeight());!0===c.p.toolbar[0]&&"bottom"!==c.p.toolbar[1]&&(e+=a(c.grid.uDiv).outerHeight());c.grid.fhDiv=a('<div style="position:absolute;left:0px;top:'+e+"px;height:"+d+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');c.grid.fbDiv=a('<div style="position:absolute;left:0px;top:'+(parseInt(e,10)+parseInt(d,10)+1)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
a("#gview_"+a.jgrid.jqID(c.p.id)).append(c.grid.fhDiv);e=a(".ui-jqgrid-htable","#gview_"+a.jgrid.jqID(c.p.id)).clone(!0);if(c.p.groupHeader){a("tr.jqg-first-row-header, tr.jqg-third-row-header",e).each(function(){a("th:gt("+b+")",this).remove()});var g=-1,i=-1;a("tr.jqg-second-row-header th",e).each(function(){var d=parseInt(a(this).attr("colspan"),10);d&&(g+=d,i++);if(g===b)return!1});g!==b&&(i=b);a("tr.jqg-second-row-header",e).each(function(){a("th:gt("+i+")",this).remove()})}else a("tr",e).each(function(){a("th:gt("+
b+")",this).remove()});a(e).width(1);a(c.grid.fhDiv).append(e).mousemove(function(a){if(c.grid.resizing)return c.grid.dragMove(a),!1});a(c).bind("jqGridResizeStop.setFrozenColumns",function(b,d,e){b=a(".ui-jqgrid-htable",c.grid.fhDiv);a("th:eq("+e+")",b).width(d);b=a(".ui-jqgrid-btable",c.grid.fbDiv);a("tr:first td:eq("+e+")",b).width(d)});a(c).bind("jqGridOnSortCol.setFrozenColumns",function(b,d,e){b=a("tr.ui-jqgrid-labels:last th:eq("+c.p.lastsort+")",c.grid.fhDiv);d=a("tr.ui-jqgrid-labels:last th:eq("+
...
h,m,n){return c.each(function(){h=h?a(h,c):c;h.bind("mousedown",{e:c,k:m},function(c){var h=c.data,j={};d=h.e;f=n?a(n):!1;if("relative"!=d.css("position"))try{d.position(j)}catch(m){}e={X:j.left||g("left")||0,Y:j.top||g("top")||0,W:g("width")||d[0].scrollWidth||0,H:g("height")||d[0].scrollHeight||0,pX:c.pageX,pY:c.pageY,k:h.k};b=f&&"d"!=h.k?{X:j.left||i("left")||0,Y:j.top||i("top")||0,W:f[0].offsetWidth||i("width")||0,H:f[0].offsetHeight||i("height")||0,pX:c.pageX,pY:c.pageY,k:h.k}:!1;if(a("input.hasDatepicker",
d[0])[0])try{a("input.hasDatepicker",d[0]).datepicker("hide")}catch(k){}a(document).mousemove(a.jqDnR.drag).mouseup(a.jqDnR.stop);return!1})})},g=function(a){return parseInt(d.css(a),10)||!1},i=function(a){return parseInt(f.css(a),10)||!1}})(jQuery);
...
f)||-1<a.inArray(a.trim(m[0]),f)))b.selected="selected"}}else if("object"===typeof e.value)for(j in n=e.value,n)if(n.hasOwnProperty(j)){b=document.createElement("option");b.setAttribute("role","option");b.value=j;b.innerHTML=n[j];g.appendChild(b);if(!c&&(a.trim(j)==a.trim(d)||a.trim(n[j])==a.trim(d)))b.selected="selected";if(c&&(-1<a.inArray(a.trim(n[j]),f)||-1<a.inArray(a.trim(j),f)))b.selected="selected"}h(g,e,["value"])}break;case "text":case "password":case "button":j="button"==c?"button":"textbox";
g=document.createElement("input");g.type=c;g.value=d;h(g,e);"button"!=c&&(f?e.size||a(g).css({width:"98%"}):e.size||(e.size=20));a(g).attr("role",j);break;case "image":case "file":g=document.createElement("input");g.type=c;h(g,e);break;case "custom":g=document.createElement("span");try{if(a.isFunction(e.custom_element))if(n=e.custom_element.call(i,d,e))n=a(n).addClass("customelement").attr({id:e.id,name:e.name}),a(g).empty().append(n);else throw"e2";else throw"e1";}catch(o){"e1"==o&&a.jgrid.info_dialog(a.jgrid.errors.errcap,
...
b.groups)for(e=0;e<b.groups.length;e++)i=a("<tr></tr>"),g.append(i),h=a("<td class='first'></td>"),i.append(h),h=a("<td colspan='4'></td>"),h.append(this.createTableForGroup(b.groups[e],b)),i.append(h);void 0===b.groupOp&&(b.groupOp=c.p.groupOps[0].op);if(void 0!==b.rules)for(e=0;e<b.rules.length;e++)g.append(this.createTableRowForRule(b.rules[e],b));return g};this.createTableRowForRule=function(b,d){var c=this,g=j(),h=a("<tr></tr>"),k,r,s,t,v="",w;h.append("<td class='first'></td>");var z=a("<td class='columns'></td>");
h.append(z);var J=a("<select></select>"),x,B=[];z.append(J);J.bind("change",function(){b.field=a(J).val();s=a(this).parents("tr:first");for(k=0;k<c.p.columns.length;k++)if(c.p.columns[k].name===b.field){t=c.p.columns[k];break}if(t){t.searchoptions.id=a.jgrid.randId();e&&"text"===t.inputtype&&!t.searchoptions.size&&(t.searchoptions.size=10);var d=a.jgrid.createEl.call(g,t.inputtype,t.searchoptions,"",!0,c.p.ajaxSelectOptions,!0);a(d).addClass("input-elm");r=t.searchoptions.sopt?t.searchoptions.sopt:
c.p.sopt?c.p.sopt:-1!==a.inArray(t.searchtype,c.p.strarr)?c.p.stropts:c.p.numopts;var f="",h=0;B=[];a.each(c.p.ops,function(){B.push(this.oper)});for(k=0;k<r.length;k++)x=a.inArray(r[k],B),-1!==x&&(0===h&&(b.op=c.p.ops[x].oper),f+="<option value='"+c.p.ops[x].oper+"'>"+c.p.ops[x].text+"</option>",h++);a(".selectopts",s).empty().append(f);a(".selectopts",s)[0].selectedIndex=0;a.jgrid.msie&&9>a.jgrid.msiever()&&(f=parseInt(a("select.selectopts",s)[0].offsetWidth,10)+1,a(".selectopts",s).width(f),a(".selectopts",
s).css("width","auto"));a(".data",s).empty().append(d);a.jgrid.bindEv.call(g,d,t.searchoptions);a(".input-elm",s).bind("change",function(d){var e=a(this).hasClass("ui-autocomplete-input")?200:0;setTimeout(function(){var e=d.target;b.data=e.nodeName.toUpperCase()==="SPAN"&&t.searchoptions&&a.isFunction(t.searchoptions.custom_value)?t.searchoptions.custom_value.call(g,a(e).children(".customelement:first"),"get"):e.value;c.onchange()},e)});setTimeout(function(){b.data=a(d).val();c.onchange()},0)}});
...
10);z=a.jgrid.createEl.call(g,t.inputtype,t.searchoptions,b.data,!0,c.p.ajaxSelectOptions,!0);if("nu"===b.op||"nn"===b.op)a(z).attr("readonly","true"),a(z).attr("disabled","true");var u=a("<select class='selectopts'></select>");v.append(u);u.bind("change",function(){b.op=a(u).val();s=a(this).parents("tr:first");var d=a(".input-elm",s)[0];if(b.op==="nu"||b.op==="nn"){b.data="";d.value="";d.setAttribute("readonly","true");d.setAttribute("disabled","true")}else{d.removeAttribute("readonly");d.removeAttribute("disabled")}c.onchange()});
r=t.searchoptions.sopt?t.searchoptions.sopt:c.p.sopt?c.p.sopt:-1!==a.inArray(t.searchtype,c.p.strarr)?c.p.stropts:c.p.numopts;v="";a.each(c.p.ops,function(){B.push(this.oper)});for(k=0;k<r.length;k++)x=a.inArray(r[k],B),-1!==x&&(w=b.op===c.p.ops[x].oper?" selected='selected'":"",v+="<option value='"+c.p.ops[x].oper+"'"+w+">"+c.p.ops[x].text+"</option>");u.append(v);v=a("<td class='data'></td>");h.append(v);v.append(z);a.jgrid.bindEv.call(g,z,t.searchoptions);a(z).addClass("input-elm").bind("change",
...
c.Find+"</a>",l="<a href='javascript:void(0)' id='"+b+"_reset' class='fm-button ui-state-default ui-corner-all fm-button-icon-left ui-search'><span class='ui-icon ui-icon-arrowreturnthick-1-w'></span>"+c.Reset+"</a>",p="",q="",r,s=!1,t=-1;c.showQuery&&(p="<a href='javascript:void(0)' id='"+b+"_query' class='fm-button ui-state-default ui-corner-all fm-button-icon-left'><span class='ui-icon ui-icon-comment'></span>Query</a>");c.columns.length?(n=c.columns,t=0,r=n[0].index||n[0].name):a.each(n,function(a,
b){if(!b.label)b.label=f.p.colNames[a];if(!s){var d=b.search===void 0?true:b.search,c=b.hidden===true;if(b.searchoptions&&b.searchoptions.searchhidden===true&&d||d&&!c){s=true;r=b.index||b.name;t=a}}});if(!i&&r||!1===c.multipleSearch){var v="eq";0<=t&&n[t].searchoptions&&n[t].searchoptions.sopt?v=n[t].searchoptions.sopt[0]:c.sopt&&c.sopt.length&&(v=c.sopt[0]);i={groupOp:"AND",rules:[{field:r,op:v,data:""}]}}s=!1;c.tmplNames&&c.tmplNames.length&&(s=!0,q=c.tmplLabel,q+="<select class='ui-template'>",
...
a.jgrid.jqID(f.p.id),jqm:c.jqModal,onClose:c.onClose});return false});a("#"+b+"_reset").bind("click",function(){var d={},g=a("#"+b);f.p.search=false;c.multipleSearch===false?d[c.sField]=d[c.sValue]=d[c.sOper]="":d[c.sFilter]="";g[0].resetFilter();s&&a(".ui-template",j).val("default");a.extend(f.p.postData,d);a(f).triggerHandler("jqGridFilterReset");a.isFunction(c.onReset)&&c.onReset.call(f);a(f).trigger("reloadGrid",[{page:1}]);return false});d(a("#"+b));a(".fm-button:not(.ui-state-disabled)",j).hover(function(){a(this).addClass("ui-state-hover")},
...
j=0,k,m,n,o=[],t=!1,s="",p;for(p=1;p<=f;p++)s+="<td class='CaptionTD'>&#160;</td><td class='DataTD'>&#160;</td>";"_empty"!=b&&(t=a(d).jqGrid("getInd",b));a(d.p.colModel).each(function(p){g=this.name;m=(i=this.editrules&&!0===this.editrules.edithidden?!1:!0===this.hidden?!0:!1)?"style='display:none'":"";if("cb"!==g&&"subgrid"!==g&&!0===this.editable&&"rn"!==g){if(!1===t)k="";else if(g==d.p.ExpandColumn&&!0===d.p.treeGrid)k=a("td[role='gridcell']:eq("+p+")",d.rows[t]).text();else{try{k=a.unformat.call(d,
a("td[role='gridcell']:eq("+p+")",d.rows[t]),{rowId:b,colModel:this},p)}catch(x){k=this.edittype&&"textarea"==this.edittype?a("td[role='gridcell']:eq("+p+")",d.rows[t]).text():a("td[role='gridcell']:eq("+p+")",d.rows[t]).html()}if(!k||"&nbsp;"==k||"&#160;"==k||1==k.length&&160==k.charCodeAt(0))k=""}var r=a.extend({},this.editoptions||{},{id:g,name:g}),v=a.extend({},{elmprefix:"",elmsuffix:"",rowabove:!1,rowcontent:""},this.formoptions||{}),w=parseInt(v.rowpos,10)||j+1,u=parseInt(2*(parseInt(v.colpos,
10)||1),10);"_empty"==b&&r.defaultValue&&(k=a.isFunction(r.defaultValue)?r.defaultValue.call(l):r.defaultValue);this.edittype||(this.edittype="text");l.p.autoencode&&(k=a.jgrid.htmlDecode(k));n=a.jgrid.createEl.call(l,this.edittype,r,k,!1,a.extend({},a.jgrid.ajaxOptions,d.p.ajaxSelectOptions||{}));if(c[l.p.id].checkOnSubmit||c[l.p.id].checkOnUpdate)c[l.p.id]._savedData[g]=k;a(n).addClass("FormElement");-1<a.inArray(this.edittype,["text","textarea","password","select"])&&a(n).addClass("ui-widget-content ui-corner-all");
h=a(e).find("tr[rowpos="+w+"]");if(v.rowabove){var B=a("<tr><td class='contentinfo' colspan='"+2*f+"'>"+v.rowcontent+"</td></tr>");a(e).append(B);B[0].rp=w}0===h.length&&(h=a("<tr "+m+" rowpos='"+w+"'></tr>").addClass("FormData").attr("id","tr_"+g),a(h).append(s),a(e).append(h),h[0].rp=w);a("td:eq("+(u-2)+")",h[0]).html(void 0===v.label?d.p.colNames[p]:v.label);a("td:eq("+(u-1)+")",h[0]).append(v.elmprefix).append(n).append(v.elmsuffix);a.isFunction(r.custom_value)&&"_empty"!==b&&r.custom_value.call(l,
a("#"+g,"#"+q),"set",k);a.jgrid.bindEv.call(l,n,r);o[j]=p;j++}});if(0<j&&(p=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(2*f-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+d.p.id+"_id' value='"+b+"'/></td></tr>"),p[0].rp=j+999,a(e).append(p),c[l.p.id].checkOnSubmit||c[l.p.id].checkOnUpdate))c[l.p.id]._savedData[d.p.id+"_id"]=b;return o}function h(b,d,e){var f,g=0,i,h,j,k,m;if(c[l.p.id].checkOnSubmit||c[l.p.id].checkOnUpdate)c[l.p.id]._savedData=
...
{rowId:b,colModel:n[h]},h)}catch(j){i="textarea"==n[h].edittype?a(this).text():a(this).html()}l.p.autoencode&&(i=a.jgrid.htmlDecode(i));if(!0===c[l.p.id].checkOnSubmit||c[l.p.id].checkOnUpdate)c[l.p.id]._savedData[f]=i;f=a.jgrid.jqID(f);switch(n[h].edittype){case "password":case "text":case "button":case "image":case "textarea":if("&nbsp;"==i||"&#160;"==i||1==i.length&&160==i.charCodeAt(0))i="";a("#"+f,"#"+e).val(i);break;case "select":var k=i.split(","),k=a.map(k,function(b){return a.trim(b)});a("#"+
f+" option","#"+e).each(function(){this.selected=!n[h].editoptions.multiple&&(a.trim(i)==a.trim(a(this).text())||k[0]==a.trim(a(this).text())||k[0]==a.trim(a(this).val()))?!0:n[h].editoptions.multiple?-1<a.inArray(a.trim(a(this).text()),k)||-1<a.inArray(a.trim(a(this).val()),k)?!0:!1:!1});break;case "checkbox":i=""+i;if(n[h].editoptions&&n[h].editoptions.value)if(n[h].editoptions.value.split(":")[0]==i)a("#"+f,"#"+e)[l.p.useProp?"prop":"attr"]({checked:!0,defaultChecked:!0});else a("#"+f,"#"+e)[l.p.useProp?
"prop":"attr"]({checked:!1,defaultChecked:!1});else i=i.toLowerCase(),0>i.search(/(false|f|0|no|n|off|undefined)/i)&&""!==i?(a("#"+f,"#"+e)[l.p.useProp?"prop":"attr"]("checked",!0),a("#"+f,"#"+e)[l.p.useProp?"prop":"attr"]("defaultChecked",!0)):(a("#"+f,"#"+e)[l.p.useProp?"prop":"attr"]("checked",!1),a("#"+f,"#"+e)[l.p.useProp?"prop":"attr"]("defaultChecked",!1));break;case "custom":try{if(n[h].editoptions&&a.isFunction(n[h].editoptions.custom_value))n[h].editoptions.custom_value.call(l,a("#"+f,"#"+
e),"set",i);else throw"e1";}catch(m){"e1"==m?a.jgrid.info_dialog(a.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.nodefined,a.jgrid.edit.bClose):a.jgrid.info_dialog(a.jgrid.errors.errcap,m.message,a.jgrid.edit.bClose)}}g++}}),0<g&&a("#id_g",s).val(b))}}function g(){a.each(l.p.colModel,function(a,b){b.editoptions&&!0===b.editoptions.NullIfEmpty&&u.hasOwnProperty(b.name)&&""===u[b.name]&&(u[b.name]="null")})}function i(){var b,e=[!0,"",""],f={},i=l.p.prmNames,j,k,m,n,o,r=a(l).triggerHandler("jqGridAddEditBeforeCheckValues",
[a("#"+q),D]);r&&"object"===typeof r&&(u=r);a.isFunction(c[l.p.id].beforeCheckValues)&&(r=c[l.p.id].beforeCheckValues.call(l,u,a("#"+q),D))&&"object"===typeof r&&(u=r);for(m in u)if(u.hasOwnProperty(m)&&(e=a.jgrid.checkValues.call(l,u[m],m),!1===e[0]))break;g();e[0]&&(f=a(l).triggerHandler("jqGridAddEditClickSubmit",[c[l.p.id],u,D]),void 0===f&&a.isFunction(c[l.p.id].onclickSubmit)&&(f=c[l.p.id].onclickSubmit.call(l,c[l.p.id],u,D)||{}),e=a(l).triggerHandler("jqGridAddEditBeforeSubmit",[u,a("#"+q),
D]),void 0===e&&(e=[!0,"",""]),e[0]&&a.isFunction(c[l.p.id].beforeSubmit)&&(e=c[l.p.id].beforeSubmit.call(l,u,a("#"+q),D)));if(e[0]&&!c[l.p.id].processing){c[l.p.id].processing=!0;a("#sData",s+"_2").addClass("ui-state-active");k=i.oper;j=i.id;u[k]="_empty"==a.trim(u[l.p.id+"_id"])?i.addoper:i.editoper;u[k]!=i.addoper?u[j]=u[l.p.id+"_id"]:void 0===u[j]&&(u[j]=u[l.p.id+"_id"]);delete u[l.p.id+"_id"];u=a.extend(u,c[l.p.id].editData,f);if(!0===l.p.treeGrid)for(o in u[k]==i.addoper&&(n=a(l).jqGrid("getGridParam",
"selrow"),u["adjacency"==l.p.treeGridModel?l.p.treeReader.parent_id_field:"parent_id"]=n),l.p.treeReader)l.p.treeReader.hasOwnProperty(o)&&(f=l.p.treeReader[o],u.hasOwnProperty(f)&&!(u[k]==i.addoper&&"parent_id_field"===o)&&delete u[f]);u[j]=a.jgrid.stripPref(l.p.idPrefix,u[j]);o=a.extend({url:c[l.p.id].url||a(l).jqGrid("getGridParam","editurl"),type:c[l.p.id].mtype,data:a.isFunction(c[l.p.id].serializeEditData)?c[l.p.id].serializeEditData.call(l,u):u,complete:function(f,g){var m;u[j]=l.p.idPrefix+
u[j];if(f.status>=300&&f.status!==304){e[0]=false;e[1]=a(l).triggerHandler("jqGridAddEditErrorTextFormat",[f,D]);e[1]=a.isFunction(c[l.p.id].errorTextFormat)?c[l.p.id].errorTextFormat.call(l,f,D):g+" Status: '"+f.statusText+"'. Error code: "+f.status}else{e=a(l).triggerHandler("jqGridAddEditAfterSubmit",[f,u,D]);e===void 0&&(e=[true,"",""]);e[0]&&a.isFunction(c[l.p.id].afterSubmit)&&(e=c[l.p.id].afterSubmit.call(l,f,u,D))}if(e[0]===false){a("#FormError>td",s).html(e[1]);a("#FormError",s).show()}else{l.p.autoencode&&
a.each(u,function(b,d){u[b]=a.jgrid.htmlDecode(d)});if(u[k]===i.addoper){e[2]||(e[2]=a.jgrid.randId());u[j]=e[2];if(c[l.p.id].reloadAfterSubmit)a(l).trigger("reloadGrid");else{l.p.treeGrid===true?a(l).jqGrid("addChildNode",e[2],n,u):a(l).jqGrid("addRowData",e[2],u,d.addedrow);if(c[l.p.id].closeAfterAdd){l.p.treeGrid!==true&&a(l).jqGrid("setSelection",e[2]);a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,onClose:c[l.p.id].onClose})}else c[l.p.id].clearAfterAdd&&
...
a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,onClose:c[l.p.id].onClose}))):a.ajax(o))}!1===e[0]&&(a("#FormError>td",s).html(e[1]),a("#FormError",s).show())}function j(a,b){var d=!1,c;for(c in a)if(a.hasOwnProperty(c)&&a[c]!=b[c]){d=!0;break}return d}function k(){var b=!0;a("#FormError",s).hide();if(c[l.p.id].checkOnUpdate&&(u={},f(),L=j(u,c[l.p.id]._savedData)))a("#"+q).data("disabled",!0),a(".confirm","#"+t.themodal).show(),b=!1;return b}function m(){var b;
if("_empty"!==e&&void 0!==l.p.savedRow&&0<l.p.savedRow.length&&a.isFunction(a.fn.jqGrid.restoreRow))for(b=0;b<l.p.savedRow.length;b++)if(l.p.savedRow[b].id==e){a(l).jqGrid("restoreRow",e);break}}function n(b,d){var c=d[1].length-1;0===b?a("#pData",s+"_2").addClass("ui-state-disabled"):void 0!==d[1][b-1]&&a("#"+a.jgrid.jqID(d[1][b-1])).hasClass("ui-state-disabled")?a("#pData",s+"_2").addClass("ui-state-disabled"):a("#pData",s+"_2").removeClass("ui-state-disabled");b==c?a("#nData",s+"_2").addClass("ui-state-disabled"):
void 0!==d[1][b+1]&&a("#"+a.jgrid.jqID(d[1][b+1])).hasClass("ui-state-disabled")?a("#nData",s+"_2").addClass("ui-state-disabled"):a("#nData",s+"_2").removeClass("ui-state-disabled")}function o(){var b=a(l).jqGrid("getDataIDs"),d=a("#id_g",s).val();return[a.inArray(d,b),b]}var l=this;if(l.grid&&e){var p=l.p.id,q="FrmGrid_"+p,r="TblGrid_"+p,s="#"+a.jgrid.jqID(r),t={themodal:"editmod"+p,modalhead:"edithd"+p,modalcontent:"editcnt"+p,scrollelm:q},v=a.isFunction(c[l.p.id].beforeShowForm)?c[l.p.id].beforeShowForm:
...
(O=!1);if(void 0!==a("#"+a.jgrid.jqID(t.themodal))[0]){x=a(l).triggerHandler("jqGridAddEditBeforeInitData",[a("#"+a.jgrid.jqID(q)),D]);void 0===x&&(x=!0);x&&z&&(x=z.call(l,a("#"+q),D));if(!1===x)return;m();a(".ui-jqdialog-title","#"+a.jgrid.jqID(t.modalhead)).html(d.caption);a("#FormError",s).hide();c[l.p.id].topinfo?(a(".topinfo",s).html(c[l.p.id].topinfo),a(".tinfo",s).show()):a(".tinfo",s).hide();c[l.p.id].bottominfo?(a(".bottominfo",s+"_2").html(c[l.p.id].bottominfo),a(".binfo",s+"_2").show()):
a(".binfo",s+"_2").hide();h(e,l,q);"_empty"==e||!c[l.p.id].viewPagerButtons?a("#pData, #nData",s+"_2").hide():a("#pData, #nData",s+"_2").show();!0===c[l.p.id].processing&&(c[l.p.id].processing=!1,a("#sData",s+"_2").removeClass("ui-state-active"));!0===a("#"+q).data("disabled")&&(a(".confirm","#"+a.jgrid.jqID(t.themodal)).hide(),a("#"+q).data("disabled",!1));a(l).triggerHandler("jqGridAddEditBeforeShowForm",[a("#"+q),D]);v&&v.call(l,a("#"+q),D);a("#"+a.jgrid.jqID(t.themodal)).data("onClose",c[l.p.id].onClose);
a.jgrid.viewModal("#"+a.jgrid.jqID(t.themodal),{gbox:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,jqM:!1,overlay:d.overlay,modal:d.modal,overlayClass:d.overlayClass});O||a("."+a.jgrid.jqID(d.overlayClass)).click(function(){if(!k())return false;a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,onClose:c[l.p.id].onClose});return false});a(l).triggerHandler("jqGridAddEditAfterShowForm",[a("#"+q),D]);w&&w.call(l,a("#"+q),D)}else{var V=isNaN(d.dataheight)?d.dataheight:
d.dataheight+"px",x=isNaN(d.datawidth)?d.datawidth:d.datawidth+"px",V=a("<form name='FormPost' id='"+q+"' class='FormGrid' onSubmit='return false;' style='width:"+x+";overflow:auto;position:relative;height:"+V+";'></form>").data("disabled",!1),Y=a("<table id='"+r+"' class='EditTable' cellspacing='0' cellpadding='0' border='0'><tbody></tbody></table>"),x=a(l).triggerHandler("jqGridAddEditBeforeInitData",[a("#"+q),D]);void 0===x&&(x=!0);x&&z&&(x=z.call(l,a("#"+q,D)));if(!1===x)return;m();a(l.p.colModel).each(function(){var a=
this.formoptions;B=Math.max(B,a?a.colpos||0:0);F=Math.max(F,a?a.rowpos||0:0)});a(V).append(Y);z=a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"+2*B+"'></td></tr>");z[0].rp=0;a(Y).append(z);z=a("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"+2*B+"'>"+c[l.p.id].topinfo+"</td></tr>");z[0].rp=0;a(Y).append(z);var x=(z="rtl"==l.p.direction?!0:!1)?"nData":"pData",W=z?"pData":"nData";b(e,l,Y,B);var x="<a href='javascript:void(0)' id='"+x+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",
...
(z?W+x:x+W)+"</td><td class='EditButton'>"+ca+Z+"</td></tr>"+("<tr style='display:none' class='binfo'><td class='bottominfo' colspan='2'>"+c[l.p.id].bottominfo+"</td></tr>"),r=r+"</tbody></table>";if(0<F){var da=[];a.each(a(Y)[0].rows,function(a,b){da[a]=b});da.sort(function(a,b){return a.rp>b.rp?1:a.rp<b.rp?-1:0});a.each(da,function(b,d){a("tbody",Y).append(d)})}d.gbox="#gbox_"+a.jgrid.jqID(p);var ea=!1;!0===d.closeOnEscape&&(d.closeOnEscape=!1,ea=!0);r=a("<div></div>").append(V).append(r);a.jgrid.createModal(t,
r,d,"#gview_"+a.jgrid.jqID(l.p.id),a("#gbox_"+a.jgrid.jqID(l.p.id))[0]);z&&(a("#pData, #nData",s+"_2").css("float","right"),a(".EditButton",s+"_2").css("text-align","left"));c[l.p.id].topinfo&&a(".tinfo",s).show();c[l.p.id].bottominfo&&a(".binfo",s+"_2").show();r=r=null;a("#"+a.jgrid.jqID(t.themodal)).keydown(function(b){var e=b.target;if(a("#"+q).data("disabled")===true)return false;if(c[l.p.id].savekey[0]===true&&b.which==c[l.p.id].savekey[1]&&e.tagName!="TEXTAREA"){a("#sData",s+"_2").trigger("click");
return false}if(b.which===27){if(!k())return false;ea&&a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:d.gbox,jqm:d.jqModal,onClose:c[l.p.id].onClose});return false}if(c[l.p.id].navkeys[0]===true){if(a("#id_g",s).val()=="_empty")return true;if(b.which==c[l.p.id].navkeys[1]){a("#pData",s+"_2").trigger("click");return false}if(b.which==c[l.p.id].navkeys[2]){a("#nData",s+"_2").trigger("click");return false}}});d.checkOnUpdate&&(a("a.ui-jqdialog-titlebar-close span","#"+a.jgrid.jqID(t.themodal)).removeClass("jqmClose"),
a("a.ui-jqdialog-titlebar-close","#"+a.jgrid.jqID(t.themodal)).unbind("click").click(function(){if(!k())return false;a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,onClose:c[l.p.id].onClose});return false}));d.saveicon=a.extend([!0,"left","ui-icon-disk"],d.saveicon);d.closeicon=a.extend([!0,"left","ui-icon-close"],d.closeicon);!0===d.saveicon[0]&&a("#sData",s+"_2").addClass("right"==d.saveicon[1]?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+
d.saveicon[2]+"'></span>");!0===d.closeicon[0]&&a("#cData",s+"_2").addClass("right"==d.closeicon[1]?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+d.closeicon[2]+"'></span>");if(c[l.p.id].checkOnSubmit||c[l.p.id].checkOnUpdate)ca="<a href='javascript:void(0)' id='sNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+d.bYes+"</a>",W="<a href='javascript:void(0)' id='nNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+d.bNo+
"</a>",Z="<a href='javascript:void(0)' id='cNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+d.bExit+"</a>",r=d.zIndex||999,r++,a("<div class='"+d.overlayClass+" jqgrid-overlay confirm' style='z-index:"+r+";display:none;'>&#160;</div><div class='confirm ui-widget-content ui-jqconfirm' style='z-index:"+(r+1)+"'>"+d.saveData+"<br/><br/>"+ca+W+Z+"</div>").insertAfter("#"+q),a("#sNew","#"+a.jgrid.jqID(t.themodal)).click(function(){i();a("#"+q).data("disabled",false);a(".confirm",
"#"+a.jgrid.jqID(t.themodal)).hide();return false}),a("#nNew","#"+a.jgrid.jqID(t.themodal)).click(function(){a(".confirm","#"+a.jgrid.jqID(t.themodal)).hide();a("#"+q).data("disabled",false);setTimeout(function(){a(":input:visible","#"+q)[0].focus()},0);return false}),a("#cNew","#"+a.jgrid.jqID(t.themodal)).click(function(){a(".confirm","#"+a.jgrid.jqID(t.themodal)).hide();a("#"+q).data("disabled",false);a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,onClose:c[l.p.id].onClose});
return false});a(l).triggerHandler("jqGridAddEditInitializeForm",[a("#"+q),D]);J&&J.call(l,a("#"+q),D);"_empty"==e||!c[l.p.id].viewPagerButtons?a("#pData,#nData",s+"_2").hide():a("#pData,#nData",s+"_2").show();a(l).triggerHandler("jqGridAddEditBeforeShowForm",[a("#"+q),D]);v&&v.call(l,a("#"+q),D);a("#"+a.jgrid.jqID(t.themodal)).data("onClose",c[l.p.id].onClose);a.jgrid.viewModal("#"+a.jgrid.jqID(t.themodal),{gbox:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,overlay:d.overlay,modal:d.modal,overlayClass:d.overlayClass});
O||a("."+a.jgrid.jqID(d.overlayClass)).click(function(){if(!k())return false;a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,onClose:c[l.p.id].onClose});return false});a(l).triggerHandler("jqGridAddEditAfterShowForm",[a("#"+q),D]);w&&w.call(l,a("#"+q),D);a(".fm-button","#"+a.jgrid.jqID(t.themodal)).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});a("#sData",s+"_2").click(function(){u={};a("#FormError",
s).hide();f();if(u[l.p.id+"_id"]=="_empty")i();else if(d.checkOnSubmit===true)if(L=j(u,c[l.p.id]._savedData)){a("#"+q).data("disabled",true);a(".confirm","#"+a.jgrid.jqID(t.themodal)).show()}else i();else i();return false});a("#cData",s+"_2").click(function(){if(!k())return false;a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(p),jqm:d.jqModal,onClose:c[l.p.id].onClose});return false});a("#nData",s+"_2").click(function(){if(!k())return false;a("#FormError",s).hide();var b=
o();b[0]=parseInt(b[0],10);if(b[0]!==-1&&b[1][b[0]+1]){a(l).triggerHandler("jqGridAddEditClickPgButtons",["next",a("#"+q),b[1][b[0]]]);var c;if(a.isFunction(d.onclickPgButtons)){c=d.onclickPgButtons.call(l,"next",a("#"+q),b[1][b[0]]);if(c!==void 0&&c===false)return false}if(a("#"+a.jgrid.jqID(b[1][b[0]+1])).hasClass("ui-state-disabled"))return false;h(b[1][b[0]+1],l,q);a(l).jqGrid("setSelection",b[1][b[0]+1]);a(l).triggerHandler("jqGridAddEditAfterClickPgButtons",["next",a("#"+q),b[1][b[0]]]);a.isFunction(d.afterclickPgButtons)&&
d.afterclickPgButtons.call(l,"next",a("#"+q),b[1][b[0]+1]);n(b[0]+1,b)}return false});a("#pData",s+"_2").click(function(){if(!k())return false;a("#FormError",s).hide();var b=o();if(b[0]!==-1&&b[1][b[0]-1]){a(l).triggerHandler("jqGridAddEditClickPgButtons",["prev",a("#"+q),b[1][b[0]]]);var c;if(a.isFunction(d.onclickPgButtons)){c=d.onclickPgButtons.call(l,"prev",a("#"+q),b[1][b[0]]);if(c!==void 0&&c===false)return false}if(a("#"+a.jgrid.jqID(b[1][b[0]-1])).hasClass("ui-state-disabled"))return false;
h(b[1][b[0]-1],l,q);a(l).jqGrid("setSelection",b[1][b[0]-1]);a(l).triggerHandler("jqGridAddEditAfterClickPgButtons",["prev",a("#"+q),b[1][b[0]]]);a.isFunction(d.afterclickPgButtons)&&d.afterclickPgButtons.call(l,"prev",a("#"+q),b[1][b[0]-1]);n(b[0]-1,b)}return false})}v=o();n(v[0],v)}})},viewGridRow:function(e,d){d=a.extend(!0,{top:0,left:0,width:0,datawidth:"auto",height:"auto",dataheight:"auto",modal:!1,overlay:30,drag:!0,resize:!0,jqModal:!0,closeOnEscape:!1,labelswidth:"30%",closeicon:[],navkeys:[!1,
38,40],onClose:null,beforeShowForm:null,beforeInitData:null,viewPagerButtons:!0,recreateForm:!1},a.jgrid.view,d||{});c[a(this)[0].p.id]=d;return this.each(function(){function f(){(!0===c[j.p.id].closeOnEscape||!0===c[j.p.id].navkeys[0])&&setTimeout(function(){a(".ui-jqdialog-titlebar-close","#"+a.jgrid.jqID(p.modalhead)).focus()},0)}function b(b,c,e,f){var g,i,h,j=0,k,l,m=[],n=!1,o,q="<td class='CaptionTD form-view-label ui-widget-content' width='"+d.labelswidth+"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>",
p="",t=["integer","number","currency"],s=0,r=0,v,w,z;for(o=1;o<=f;o++)p+=1==o?q:"<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";a(c.p.colModel).each(function(){i=this.editrules&&!0===this.editrules.edithidden?!1:!0===this.hidden?!0:!1;!i&&"right"===this.align&&(this.formatter&&-1!==a.inArray(this.formatter,t)?s=Math.max(s,parseInt(this.width,10)):r=Math.max(r,parseInt(this.width,10)))});v=0!==s?s:0!==r?r:0;n=a(c).jqGrid("getInd",
b);a(c.p.colModel).each(function(b){g=this.name;w=!1;l=(i=this.editrules&&!0===this.editrules.edithidden?!1:!0===this.hidden?!0:!1)?"style='display:none'":"";z="boolean"!==typeof this.viewable?!0:this.viewable;if("cb"!==g&&"subgrid"!==g&&"rn"!==g&&z){k=!1===n?"":g==c.p.ExpandColumn&&!0===c.p.treeGrid?a("td:eq("+b+")",c.rows[n]).text():a("td:eq("+b+")",c.rows[n]).html();w="right"===this.align&&0!==v?!0:!1;var d=a.extend({},{rowabove:!1,rowcontent:""},this.formoptions||{}),o=parseInt(d.rowpos,10)||
j+1,q=parseInt(2*(parseInt(d.colpos,10)||1),10);if(d.rowabove){var t=a("<tr><td class='contentinfo' colspan='"+2*f+"'>"+d.rowcontent+"</td></tr>");a(e).append(t);t[0].rp=o}h=a(e).find("tr[rowpos="+o+"]");0===h.length&&(h=a("<tr "+l+" rowpos='"+o+"'></tr>").addClass("FormData").attr("id","trv_"+g),a(h).append(p),a(e).append(h),h[0].rp=o);a("td:eq("+(q-2)+")",h[0]).html("<b>"+(void 0===d.label?c.p.colNames[b]:d.label)+"</b>");a("td:eq("+(q-1)+")",h[0]).append("<span>"+k+"</span>").attr("id","v_"+g);
w&&a("td:eq("+(q-1)+") span",h[0]).css({"text-align":"right",width:v+"px"});m[j]=b;j++}});0<j&&(b=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(2*f-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+b+"'/></td></tr>"),b[0].rp=j+99,a(e).append(b));return m}function h(b,d){var c,e,f=0,g,i;if(i=a(d).jqGrid("getInd",b,!0))a("td",i).each(function(b){c=d.p.colModel[b].name;e=d.p.colModel[b].editrules&&!0===d.p.colModel[b].editrules.edithidden?
!1:!0===d.p.colModel[b].hidden?!0:!1;"cb"!==c&&"subgrid"!==c&&"rn"!==c&&(g=c==d.p.ExpandColumn&&!0===d.p.treeGrid?a(this).text():a(this).html(),a.extend({},d.p.colModel[b].editoptions||{}),c=a.jgrid.jqID("v_"+c),a("#"+c+" span","#"+n).html(g),e&&a("#"+c,"#"+n).parents("tr:first").hide(),f++)}),0<f&&a("#id_g","#"+n).val(b)}function g(b,d){var c=d[1].length-1;0===b?a("#pData","#"+n+"_2").addClass("ui-state-disabled"):void 0!==d[1][b-1]&&a("#"+a.jgrid.jqID(d[1][b-1])).hasClass("ui-state-disabled")?a("#pData",
n+"_2").addClass("ui-state-disabled"):a("#pData","#"+n+"_2").removeClass("ui-state-disabled");b==c?a("#nData","#"+n+"_2").addClass("ui-state-disabled"):void 0!==d[1][b+1]&&a("#"+a.jgrid.jqID(d[1][b+1])).hasClass("ui-state-disabled")?a("#nData",n+"_2").addClass("ui-state-disabled"):a("#nData","#"+n+"_2").removeClass("ui-state-disabled")}function i(){var b=a(j).jqGrid("getDataIDs"),d=a("#id_g","#"+n).val();return[a.inArray(d,b),b]}var j=this;if(j.grid&&e){var k=j.p.id,m="ViewGrid_"+a.jgrid.jqID(k),
n="ViewTbl_"+a.jgrid.jqID(k),o="ViewGrid_"+k,l="ViewTbl_"+k,p={themodal:"viewmod"+k,modalhead:"viewhd"+k,modalcontent:"viewcnt"+k,scrollelm:m},q=a.isFunction(c[j.p.id].beforeInitData)?c[j.p.id].beforeInitData:!1,r=!0,s=1,t=0;!0===d.recreateForm&&void 0!==a("#"+a.jgrid.jqID(p.themodal))[0]&&a("#"+a.jgrid.jqID(p.themodal)).remove();if(void 0!==a("#"+a.jgrid.jqID(p.themodal))[0]){q&&(r=q.call(j,a("#"+m)),void 0===r&&(r=!0));if(!1===r)return;a(".ui-jqdialog-title","#"+a.jgrid.jqID(p.modalhead)).html(d.caption);
a("#FormError","#"+n).hide();h(e,j);a.isFunction(c[j.p.id].beforeShowForm)&&c[j.p.id].beforeShowForm.call(j,a("#"+m));a.jgrid.viewModal("#"+a.jgrid.jqID(p.themodal),{gbox:"#gbox_"+a.jgrid.jqID(k),jqm:d.jqModal,jqM:!1,overlay:d.overlay,modal:d.modal});f()}else{var v=isNaN(d.dataheight)?d.dataheight:d.dataheight+"px",w=isNaN(d.datawidth)?d.datawidth:d.datawidth+"px",o=a("<form name='FormPost' id='"+o+"' class='FormGrid' style='width:"+w+";overflow:auto;position:relative;height:"+v+";'></form>"),z=a("<table id='"+
l+"' class='EditTable' cellspacing='1' cellpadding='2' border='0' style='table-layout:fixed'><tbody></tbody></table>");q&&(r=q.call(j,a("#"+m)),void 0===r&&(r=!0));if(!1===r)return;a(j.p.colModel).each(function(){var a=this.formoptions;s=Math.max(s,a?a.colpos||0:0);t=Math.max(t,a?a.rowpos||0:0)});a(o).append(z);b(e,j,z,s);l="rtl"==j.p.direction?!0:!1;q="<a href='javascript:void(0)' id='"+(l?"nData":"pData")+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>";
...
!1,i=f.p.id,j={},k=!0,m="DelTbl_"+a.jgrid.jqID(i),n,o,l,p,q="DelTbl_"+i,r={themodal:"delmod"+i,modalhead:"delhd"+i,modalcontent:"delcnt"+i,scrollelm:m};a.isArray(e)&&(e=e.join());if(void 0!==a("#"+a.jgrid.jqID(r.themodal))[0]){g&&(k=g.call(f,a("#"+m)),void 0===k&&(k=!0));if(!1===k)return;a("#DelData>td","#"+m).text(e);a("#DelError","#"+m).hide();!0===c[f.p.id].processing&&(c[f.p.id].processing=!1,a("#dData","#"+m).removeClass("ui-state-active"));b&&c[f.p.id].beforeShowForm.call(f,a("#"+m));a.jgrid.viewModal("#"+
a.jgrid.jqID(r.themodal),{gbox:"#gbox_"+a.jgrid.jqID(i),jqm:c[f.p.id].jqModal,jqM:!1,overlay:c[f.p.id].overlay,modal:c[f.p.id].modal})}else{var s=isNaN(c[f.p.id].dataheight)?c[f.p.id].dataheight:c[f.p.id].dataheight+"px",t=isNaN(d.datawidth)?d.datawidth:d.datawidth+"px",q="<div id='"+q+"' class='formdata' style='width:"+t+";overflow:auto;position:relative;height:"+s+";'><table class='DelTable'><tbody><tr id='DelError' style='display:none'><td class='ui-state-error'></td></tr>"+("<tr id='DelData' style='display:none'><td >"+
...
a.jgrid.jqID(k.p.id),a("#gbox_"+a.jgrid.jqID(k.p.id))[0],!0));var n=1,o,l=function(){a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")},p=function(){a(this).removeClass("ui-state-hover")};d.cloneToTop&&k.p.toppager&&(n=2);for(o=0;o<n;o++){var q=a("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left;table-layout:auto;'><tbody><tr></tr></tbody></table>"),r,s;0===o?(r=c,s=k.p.id,r==k.p.toppager&&(s+="_top",n=1)):(r=k.p.toppager,s=k.p.id+
"_top");"rtl"==k.p.direction&&a(q).attr("dir","rtl").css("float","right");d.add&&(b=b||{},m=a("<td class='ui-pg-button ui-corner-all'></td>"),a(m).append("<div class='ui-pg-div'><span class='ui-icon "+d.addicon+"'></span>"+d.addtext+"</div>"),a("tr",q).append(m),a(m,q).attr({title:d.addtitle||"",id:b.id||"add_"+s}).click(function(){a(this).hasClass("ui-state-disabled")||(a.isFunction(d.addfunc)?d.addfunc.call(k):a(k).jqGrid("editGridRow","new",b));return false}).hover(l,p),m=null);d.edit&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),
f=f||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+d.editicon+"'></span>"+d.edittext+"</div>"),a("tr",q).append(m),a(m,q).attr({title:d.edittitle||"",id:f.id||"edit_"+s}).click(function(){if(!a(this).hasClass("ui-state-disabled")){var b=k.p.selrow;if(b)a.isFunction(d.editfunc)?d.editfunc.call(k,b):a(k).jqGrid("editGridRow",b,f);else{a.jgrid.viewModal("#"+j.themodal,{gbox:"#gbox_"+a.jgrid.jqID(k.p.id),jqm:true});a("#jqg_alrt").focus()}}return false}).hover(l,p),m=null);d.view&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),
i=i||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+d.viewicon+"'></span>"+d.viewtext+"</div>"),a("tr",q).append(m),a(m,q).attr({title:d.viewtitle||"",id:i.id||"view_"+s}).click(function(){if(!a(this).hasClass("ui-state-disabled")){var b=k.p.selrow;if(b)a.isFunction(d.viewfunc)?d.viewfunc.call(k,b):a(k).jqGrid("viewGridRow",b,i);else{a.jgrid.viewModal("#"+j.themodal,{gbox:"#gbox_"+a.jgrid.jqID(k.p.id),jqm:true});a("#jqg_alrt").focus()}}return false}).hover(l,p),m=null);d.del&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),
h=h||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+d.delicon+"'></span>"+d.deltext+"</div>"),a("tr",q).append(m),a(m,q).attr({title:d.deltitle||"",id:h.id||"del_"+s}).click(function(){if(!a(this).hasClass("ui-state-disabled")){var b;if(k.p.multiselect){b=k.p.selarrrow;b.length===0&&(b=null)}else b=k.p.selrow;if(b)a.isFunction(d.delfunc)?d.delfunc.call(k,b):a(k).jqGrid("delGridRow",b,h);else{a.jgrid.viewModal("#"+j.themodal,{gbox:"#gbox_"+a.jgrid.jqID(k.p.id),jqm:true});a("#jqg_alrt").focus()}}return false}).hover(l,
p),m=null);(d.add||d.edit||d.del||d.view)&&a("tr",q).append("<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>");d.search&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),g=g||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+d.searchicon+"'></span>"+d.searchtext+"</div>"),a("tr",q).append(m),a(m,q).attr({title:d.searchtitle||"",id:g.id||"search_"+s}).click(function(){a(this).hasClass("ui-state-disabled")||(a.isFunction(d.searchfunc)?
d.searchfunc.call(k,g):a(k).jqGrid("searchGrid",g));return false}).hover(l,p),g.showOnLoad&&!0===g.showOnLoad&&a(m,q).click(),m=null);d.refresh&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),a(m).append("<div class='ui-pg-div'><span class='ui-icon "+d.refreshicon+"'></span>"+d.refreshtext+"</div>"),a("tr",q).append(m),a(m,q).attr({title:d.refreshtitle||"",id:"refresh_"+s}).click(function(){if(!a(this).hasClass("ui-state-disabled")){a.isFunction(d.beforeRefresh)&&d.beforeRefresh.call(k);k.p.search=
false;try{var b=k.p.id;k.p.postData.filters="";try{a("#fbox_"+a.jgrid.jqID(b)).jqFilter("resetFilter")}catch(c){}a.isFunction(k.clearToolbar)&&k.clearToolbar.call(k,false)}catch(e){}switch(d.refreshstate){case "firstpage":a(k).trigger("reloadGrid",[{page:1}]);break;case "current":a(k).trigger("reloadGrid",[{current:true}])}a.isFunction(d.afterRefresh)&&d.afterRefresh.call(k)}return false}).hover(l,p),m=null);m=a(".ui-jqgrid").css("font-size")||"11px";a("body").append("<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+
m+";visibility:hidden;' ></div>");m=a(q).clone().appendTo("#testpg2").width();a("#testpg2").remove();a(r+"_"+d.position,r).append(q);k.p._nvtd&&(m>k.p._nvtd[0]&&(a(r+"_"+d.position,r).width(m),k.p._nvtd[0]=m),k.p._nvtd[1]=m);q=m=m=null;this.nav=!0}}}})},navButtonAdd:function(c,d){d=a.extend({caption:"newButton",title:"",buttonicon:"ui-icon-newwin",onClickButton:null,position:"last",cursor:"pointer"},d||{});return this.each(function(){if(this.grid){"string"===typeof c&&0!==c.indexOf("#")&&(c="#"+a.jgrid.jqID(c));
var f=a(".navtable",c)[0],b=this;if(f&&!(d.id&&void 0!==a("#"+a.jgrid.jqID(d.id),f)[0])){var h=a("<td></td>");"NONE"==d.buttonicon.toString().toUpperCase()?a(h).addClass("ui-pg-button ui-corner-all").append("<div class='ui-pg-div'>"+d.caption+"</div>"):a(h).addClass("ui-pg-button ui-corner-all").append("<div class='ui-pg-div'><span class='ui-icon "+d.buttonicon+"'></span>"+d.caption+"</div>");d.id&&a(h).attr("id",d.id);"first"==d.position?0===f.rows[0].cells.length?a("tr",f).append(h):a("tr td:eq(0)",
f).before(h):a("tr",f).append(h);a(h,f).attr("title",d.title||"").click(function(c){a(this).hasClass("ui-state-disabled")||a.isFunction(d.onClickButton)&&d.onClickButton.call(b,c);return!1}).hover(function(){a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")})}}})},navSeparatorAdd:function(c,d){d=a.extend({sepclass:"ui-separator",sepcontent:"",position:"last"},d||{});return this.each(function(){if(this.grid){"string"===typeof c&&
0!==c.indexOf("#")&&(c="#"+a.jgrid.jqID(c));var f=a(".navtable",c)[0];if(f){var b="<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='"+d.sepclass+"'></span>"+d.sepcontent+"</td>";"first"===d.position?0===f.rows[0].cells.length?a("tr",f).append(b):a("tr td:eq(0)",f).before(b):a("tr",f).append(b)}}})},GridToForm:function(c,d){return this.each(function(){var f=this,b;if(f.grid){var h=a(f).jqGrid("getRowData",c);if(h)for(b in h)h.hasOwnProperty(b)&&(a("[name="+a.jgrid.jqID(b)+
"]",d).is("input:radio")||a("[name="+a.jgrid.jqID(b)+"]",d).is("input:checkbox")?a("[name="+a.jgrid.jqID(b)+"]",d).each(function(){if(a(this).val()==h[b])a(this)[f.p.useProp?"prop":"attr"]("checked",!0);else a(this)[f.p.useProp?"prop":"attr"]("checked",!1)}):a("[name="+a.jgrid.jqID(b)+"]",d).val(h[b]))}})},FormToGrid:function(c,d,f,b){return this.each(function(){if(this.grid){f||(f="set");b||(b="first");var h=a(d).serializeArray(),g={};a.each(h,function(a,b){g[b.name]=b.value});"add"==f?a(this).jqGrid("addRowData",
c,g,b):"set"==f&&a(this).jqGrid("setRowData",c,g)}})}})})(jQuery);
...
"function 'custom_value' "+a.jgrid.edit.msg.nodefined,a.jgrid.edit.bClose),"e2"===e?a.jgrid.info_dialog(a.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.novalue,a.jgrid.edit.bClose):a.jgrid.info_dialog(a.jgrid.errors.errcap,e.message,a.jgrid.edit.bClose)}}r=a.jgrid.checkValues.call(m,o[n],b);if(!1===r[0])return r[1]=o[n]+" "+r[1],!1;m.p.autoencode&&(o[n]=a.jgrid.htmlEncode(o[n]));"clientArray"!==j.url&&t.editoptions&&!0===t.editoptions.NullIfEmpty&&""===o[n]&&(p[n]="null")}});if(!1===
r[0]){try{var v=m.rows.namedItem(c),w=a.jgrid.findPos(v);a.jgrid.info_dialog(a.jgrid.errors.errcap,r[1],a.jgrid.edit.bClose,{left:w[0],top:w[1]+a(v).outerHeight()})}catch(z){alert(r[1])}return k}i=m.p.prmNames;v=c;w=!1===m.p.keyIndex?i.id:m.p.colModel[m.p.keyIndex+(!0===m.p.rownumbers?1:0)+(!0===m.p.multiselect?1:0)+(!0===m.p.subGrid?1:0)].name;if(o){o[i.oper]=i.editoper;if(void 0===o[w])o[w]=c;else if(s.id!==m.p.idPrefix+o[w]&&(i=a.jgrid.stripPref(m.p.idPrefix,c),void 0!==m.p._index[i]&&(m.p._index[o[w]]=
m.p._index[i],delete m.p._index[i]),c=m.p.idPrefix+o[w],a(s).attr("id",c),m.p.selrow===v&&(m.p.selrow=c),a.isArray(m.p.selarrrow)&&(i=a.inArray(v,m.p.selarrrow),0<=i&&(m.p.selarrrow[i]=c)),m.p.multiselect))i="jqg_"+m.p.id+"_"+c,a("input.cbox",s).attr("id",i).attr("name",i);void 0===m.p.inlineData&&(m.p.inlineData={});o=a.extend({},o,m.p.inlineData,j.extraparam)}if("clientArray"===j.url){o=a.extend({},o,l);m.p.autoencode&&a.each(o,function(b,d){o[b]=a.jgrid.htmlDecode(d)});i=a(m).jqGrid("setRowData",
c,o);a(s).attr("editable","0");for(w=0;w<m.p.savedRow.length;w++)if(""+m.p.savedRow[w].id===""+v){q=w;break}0<=q&&m.p.savedRow.splice(q,1);a(m).triggerHandler("jqGridInlineAfterSaveRow",[c,i,o,j]);a.isFunction(j.aftersavefunc)&&j.aftersavefunc.call(m,c,i,j);k=!0;a(s).removeClass("jqgrid-new-row").unbind("keydown")}else a("#lui_"+a.jgrid.jqID(m.p.id)).show(),p=a.extend({},o,p),p[w]=a.jgrid.stripPref(m.p.idPrefix,p[w]),a.ajax(a.extend({url:j.url,data:a.isFunction(m.p.serializeRowData)?m.p.serializeRowData.call(m,
p):p,type:j.mtype,async:!1,complete:function(b,d){a("#lui_"+a.jgrid.jqID(m.p.id)).hide();if("success"===d){var e=!0,f;f=a(m).triggerHandler("jqGridInlineSuccessSaveRow",[b,c,j]);a.isArray(f)||(f=[!0,o]);f[0]&&a.isFunction(j.successfunc)&&(f=j.successfunc.call(m,b));a.isArray(f)?(e=f[0],o=f[1]||o):e=f;if(!0===e){m.p.autoencode&&a.each(o,function(b,d){o[b]=a.jgrid.htmlDecode(d)});o=a.extend({},o,l);a(m).jqGrid("setRowData",c,o);a(s).attr("editable","0");for(e=0;e<m.p.savedRow.length;e++)if(""+m.p.savedRow[e].id===
""+c){q=e;break}0<=q&&m.p.savedRow.splice(q,1);a(m).triggerHandler("jqGridInlineAfterSaveRow",[c,b,o,j]);a.isFunction(j.aftersavefunc)&&j.aftersavefunc.call(m,c,b);k=!0;a(s).removeClass("jqgrid-new-row").unbind("keydown")}else a(m).triggerHandler("jqGridInlineErrorSaveRow",[c,b,d,null,j]),a.isFunction(j.errorfunc)&&j.errorfunc.call(m,c,b,d,null),!0===j.restoreAfterError&&a(m).jqGrid("restoreRow",c,j.afterrestorefunc)}},error:function(b,d,e){a("#lui_"+a.jgrid.jqID(m.p.id)).hide();a(m).triggerHandler("jqGridInlineErrorSaveRow",
...
(function(a){a.jgrid.extend({setTreeNode:function(c,e){return this.each(function(){var d=this;if(d.grid&&d.p.treeGrid)for(var f=d.p.expColInd,b=d.p.treeReader.expanded_field,h=d.p.treeReader.leaf_field,g=d.p.treeReader.level_field,i=d.p.treeReader.icon_field,j=d.p.treeReader.loaded,k,m,n,o;c<e;)o=d.p.data[d.p._index[d.rows[c].id]],"nested"==d.p.treeGridModel&&!o[h]&&(k=parseInt(o[d.p.treeReader.left_field],10),m=parseInt(o[d.p.treeReader.right_field],10),o[h]=m===k+1?"true":"false",d.rows[c].cells[d.p._treeleafpos].innerHTML=
o[h]),k=parseInt(o[g],10),0===d.p.tree_root_level?(n=k+1,m=k):(n=k,m=k-1),n="<div class='tree-wrap tree-wrap-"+d.p.direction+"' style='width:"+18*n+"px;'>",n+="<div style='"+("rtl"==d.p.direction?"right:":"left:")+18*m+"px;' class='ui-icon ",void 0!==o[j]&&(o[j]="true"==o[j]||!0===o[j]?!0:!1),"true"==o[h]||!0===o[h]?(n+=(void 0!==o[i]&&""!==o[i]?o[i]:d.p.treeIcons.leaf)+" tree-leaf treeclick",o[h]=!0,m="leaf"):(o[h]=!1,m=""),o[b]=("true"==o[b]||!0===o[b]?!0:!1)&&(o[j]||void 0===o[j]),n=!1===o[b]?
n+(!0===o[h]?"'":d.p.treeIcons.plus+" tree-plus treeclick'"):n+(!0===o[h]?"'":d.p.treeIcons.minus+" tree-minus treeclick'"),n+="></div></div>",a(d.rows[c].cells[f]).wrapInner("<span class='cell-wrapper"+m+"'></span>").prepend(n),k!==parseInt(d.p.tree_root_level,10)&&((o=(o=a(d).jqGrid("getNodeParent",o))&&o.hasOwnProperty(b)?o[b]:!0)||a(d.rows[c]).css("display","none")),a(d.rows[c].cells[f]).find("div.treeclick").bind("click",function(c){c=a(c.target||c.srcElement,d.rows).closest("tr.jqgrow")[0].id;
c=d.p._index[c];if(!d.p.data[c][h])if(d.p.data[c][b]){a(d).jqGrid("collapseRow",d.p.data[c]);a(d).jqGrid("collapseNode",d.p.data[c])}else{a(d).jqGrid("expandRow",d.p.data[c]);a(d).jqGrid("expandNode",d.p.data[c])}return false}),!0===d.p.ExpandColClick&&a(d.rows[c].cells[f]).find("span.cell-wrapper").css("cursor","pointer").bind("click",function(c){var c=a(c.target||c.srcElement,d.rows).closest("tr.jqgrow")[0].id,e=d.p._index[c];if(!d.p.data[e][h])if(d.p.data[e][b]){a(d).jqGrid("collapseRow",d.p.data[e]);
a(d).jqGrid("collapseNode",d.p.data[e])}else{a(d).jqGrid("expandRow",d.p.data[e]);a(d).jqGrid("expandNode",d.p.data[e])}a(d).jqGrid("setSelection",c);return false}),c++})},setTreeGrid:function(){return this.each(function(){var c=this,e=0,d=!1,f,b,h,g=[];if(c.p.treeGrid){c.p.treedatatype||a.extend(c.p,{treedatatype:c.p.datatype});c.p.subGrid=!1;c.p.altRows=!1;c.p.pgbuttons=!1;c.p.pginput=!1;c.p.gridview=!0;null===c.p.rowTotal&&(c.p.rowNum=1E4);c.p.multiselect=!1;c.p.rowList=[];c.p.expColInd=0;c.p.treeIcons=
a.extend({plus:"ui-icon-triangle-1-"+("rtl"==c.p.direction?"w":"e"),minus:"ui-icon-triangle-1-s",leaf:"ui-icon-radio-off"},c.p.treeIcons||{});"nested"==c.p.treeGridModel?c.p.treeReader=a.extend({level_field:"level",left_field:"lft",right_field:"rgt",leaf_field:"isLeaf",expanded_field:"expanded",loaded:"loaded",icon_field:"icon"},c.p.treeReader):"adjacency"==c.p.treeGridModel&&(c.p.treeReader=a.extend({level_field:"level",parent_id_field:"parent",leaf_field:"isLeaf",expanded_field:"expanded",loaded:"loaded",
icon_field:"icon"},c.p.treeReader));for(b in c.p.colModel)if(c.p.colModel.hasOwnProperty(b))for(h in f=c.p.colModel[b].name,f==c.p.ExpandColumn&&!d&&(d=!0,c.p.expColInd=e),e++,c.p.treeReader)c.p.treeReader.hasOwnProperty(h)&&c.p.treeReader[h]==f&&g.push(f);a.each(c.p.treeReader,function(b,d){if(d&&a.inArray(d,g)===-1){if(b==="leaf_field")c.p._treeleafpos=e;e++;c.p.colNames.push(d);c.p.colModel.push({name:d,width:1,hidden:true,sortable:false,resizable:false,hidedlg:true,editable:true,search:false})}})}})},
expandRow:function(c){this.each(function(){var e=this;if(e.grid&&e.p.treeGrid){var d=a(e).jqGrid("getNodeChildren",c),f=e.p.treeReader.expanded_field,b=e.rows;a(d).each(function(){var d=a.jgrid.getAccessor(this,e.p.localReader.id);a(b.namedItem(d)).css("display","");this[f]&&a(e).jqGrid("expandRow",this)})}})},collapseRow:function(c){this.each(function(){var e=this;if(e.grid&&e.p.treeGrid){var d=a(e).jqGrid("getNodeChildren",c),f=e.p.treeReader.expanded_field,b=e.rows;a(d).each(function(){var d=a.jgrid.getAccessor(this,
e.p.localReader.id);a(b.namedItem(d)).css("display","none");this[f]&&a(e).jqGrid("collapseRow",this)})}})},getRootNodes:function(){var c=[];this.each(function(){var e=this;if(e.grid&&e.p.treeGrid)switch(e.p.treeGridModel){case "nested":var d=e.p.treeReader.level_field;a(e.p.data).each(function(){parseInt(this[d],10)===parseInt(e.p.tree_root_level,10)&&c.push(this)});break;case "adjacency":var f=e.p.treeReader.parent_id_field;a(e.p.data).each(function(){(null===this[f]||"null"==(""+this[f]).toLowerCase())&&
c.push(this)})}});return c},getNodeDepth:function(c){var e=null;this.each(function(){if(this.grid&&this.p.treeGrid)switch(this.p.treeGridModel){case "nested":e=parseInt(c[this.p.treeReader.level_field],10)-parseInt(this.p.tree_root_level,10);break;case "adjacency":e=a(this).jqGrid("getNodeAncestors",c).length}});return e},getNodeParent:function(c){var e=null;this.each(function(){var d=this;if(d.grid&&d.p.treeGrid)switch(d.p.treeGridModel){case "nested":var f=d.p.treeReader.left_field,b=d.p.treeReader.right_field,
h=d.p.treeReader.level_field,g=parseInt(c[f],10),i=parseInt(c[b],10),j=parseInt(c[h],10);a(this.p.data).each(function(){if(parseInt(this[h],10)===j-1&&parseInt(this[f],10)<g&&parseInt(this[b],10)>i)return e=this,!1});break;case "adjacency":var k=d.p.treeReader.parent_id_field,m=d.p.localReader.id;a(this.p.data).each(function(){if(this[m]===a.jgrid.stripPref(d.p.idPrefix,c[k]))return e=this,!1})}});return e},getNodeChildren:function(c){var e=[];this.each(function(){var d=this;if(d.grid&&d.p.treeGrid)switch(d.p.treeGridModel){case "nested":var f=
d.p.treeReader.left_field,b=d.p.treeReader.right_field,h=d.p.treeReader.level_field,g=parseInt(c[f],10),i=parseInt(c[b],10),j=parseInt(c[h],10);a(this.p.data).each(function(){parseInt(this[h],10)===j+1&&parseInt(this[f],10)>g&&parseInt(this[b],10)<i&&e.push(this)});break;case "adjacency":var k=d.p.treeReader.parent_id_field,m=d.p.localReader.id;a(this.p.data).each(function(){this[k]===a.jgrid.stripPref(d.p.idPrefix,c[m])&&e.push(this)})}});return e},getFullTreeNode:function(c){var e=[];this.each(function(){var d=
this,f;if(d.grid&&d.p.treeGrid)switch(d.p.treeGridModel){case "nested":var b=d.p.treeReader.left_field,h=d.p.treeReader.right_field,g=d.p.treeReader.level_field,i=parseInt(c[b],10),j=parseInt(c[h],10),k=parseInt(c[g],10);a(this.p.data).each(function(){parseInt(this[g],10)>=k&&parseInt(this[b],10)>=i&&parseInt(this[b],10)<=j&&e.push(this)});break;case "adjacency":if(c){e.push(c);var m=d.p.treeReader.parent_id_field,n=d.p.localReader.id;a(this.p.data).each(function(b){f=e.length;for(b=0;b<f;b++)if(a.jgrid.stripPref(d.p.idPrefix,
e[b][n])===this[m]){e.push(this);break}})}}});return e},getNodeAncestors:function(c){var e=[];this.each(function(){if(this.grid&&this.p.treeGrid)for(var d=a(this).jqGrid("getNodeParent",c);d;)e.push(d),d=a(this).jqGrid("getNodeParent",d)});return e},isVisibleNode:function(c){var e=!0;this.each(function(){if(this.grid&&this.p.treeGrid){var d=a(this).jqGrid("getNodeAncestors",c),f=this.p.treeReader.expanded_field;a(d).each(function(){e=e&&this[f];if(!e)return!1})}});return e},isNodeLoaded:function(c){var e;
this.each(function(){if(this.grid&&this.p.treeGrid){var d=this.p.treeReader.leaf_field;e=void 0!==c?void 0!==c.loaded?c.loaded:c[d]||0<a(this).jqGrid("getNodeChildren",c).length?!0:!1:!1}});return e},expandNode:function(c){return this.each(function(){if(this.grid&&this.p.treeGrid){var e=this.p.treeReader.expanded_field,d=this.p.treeReader.parent_id_field,f=this.p.treeReader.loaded,b=this.p.treeReader.level_field,h=this.p.treeReader.left_field,g=this.p.treeReader.right_field;if(!c[e]){var i=a.jgrid.getAccessor(c,
this.p.localReader.id),j=a("#"+a.jgrid.jqID(i),this.grid.bDiv)[0],k=this.p._index[i];a(this).jqGrid("isNodeLoaded",this.p.data[k])?(c[e]=!0,a("div.treeclick",j).removeClass(this.p.treeIcons.plus+" tree-plus").addClass(this.p.treeIcons.minus+" tree-minus")):this.grid.hDiv.loading||(c[e]=!0,a("div.treeclick",j).removeClass(this.p.treeIcons.plus+" tree-plus").addClass(this.p.treeIcons.minus+" tree-minus"),this.p.treeANode=j.rowIndex,this.p.datatype=this.p.treedatatype,"nested"==this.p.treeGridModel?
a(this).jqGrid("setGridParam",{postData:{nodeid:i,n_left:c[h],n_right:c[g],n_level:c[b]}}):a(this).jqGrid("setGridParam",{postData:{nodeid:i,parentid:c[d],n_level:c[b]}}),a(this).trigger("reloadGrid"),c[f]=!0,"nested"==this.p.treeGridModel?a(this).jqGrid("setGridParam",{postData:{nodeid:"",n_left:"",n_right:"",n_level:""}}):a(this).jqGrid("setGridParam",{postData:{nodeid:"",parentid:"",n_level:""}}))}}})},collapseNode:function(c){return this.each(function(){if(this.grid&&this.p.treeGrid){var e=this.p.treeReader.expanded_field;
c[e]&&(c[e]=!1,e=a.jgrid.getAccessor(c,this.p.localReader.id),e=a("#"+a.jgrid.jqID(e),this.grid.bDiv)[0],a("div.treeclick",e).removeClass(this.p.treeIcons.minus+" tree-minus").addClass(this.p.treeIcons.plus+" tree-plus"))}})},SortTree:function(c,e,d,f){return this.each(function(){if(this.grid&&this.p.treeGrid){var b,h,g,i=[],j=this,k;b=a(this).jqGrid("getRootNodes");b=a.jgrid.from(b);b.orderBy(c,e,d,f);k=b.select();b=0;for(h=k.length;b<h;b++)g=k[b],i.push(g),a(this).jqGrid("collectChildrenSortTree",
i,g,c,e,d,f);a.each(i,function(b){var d=a.jgrid.getAccessor(this,j.p.localReader.id);a("#"+a.jgrid.jqID(j.p.id)+" tbody tr:eq("+b+")").after(a("tr#"+a.jgrid.jqID(d),j.grid.bDiv))});i=k=b=null}})},collectChildrenSortTree:function(c,e,d,f,b,h){return this.each(function(){if(this.grid&&this.p.treeGrid){var g,i,j,k;g=a(this).jqGrid("getNodeChildren",e);g=a.jgrid.from(g);g.orderBy(d,f,b,h);k=g.select();g=0;for(i=k.length;g<i;g++)j=k[g],c.push(j),a(this).jqGrid("collectChildrenSortTree",c,j,d,f,b,h)}})},
setTreeRow:function(c,e){var d=!1;this.each(function(){this.grid&&this.p.treeGrid&&(d=a(this).jqGrid("setRowData",c,e))});return d},delTreeNode:function(c){return this.each(function(){var e=this.p.localReader.id,d,f=this.p.treeReader.left_field,b=this.p.treeReader.right_field,h,g,i;if(this.grid&&this.p.treeGrid&&(d=this.p._index[c],void 0!==d)){h=parseInt(this.p.data[d][b],10);g=h-parseInt(this.p.data[d][f],10)+1;var j=a(this).jqGrid("getFullTreeNode",this.p.data[d]);if(0<j.length)for(d=0;d<j.length;d++)a(this).jqGrid("delRowData",
j[d][e]);if("nested"===this.p.treeGridModel){e=a.jgrid.from(this.p.data).greater(f,h,{stype:"integer"}).select();if(e.length)for(i in e)e.hasOwnProperty(i)&&(e[i][f]=parseInt(e[i][f],10)-g);e=a.jgrid.from(this.p.data).greater(b,h,{stype:"integer"}).select();if(e.length)for(i in e)e.hasOwnProperty(i)&&(e[i][b]=parseInt(e[i][b],10)-g)}}})},addChildNode:function(c,e,d,f){var b=this[0];if(d){var h=b.p.treeReader.expanded_field,g=b.p.treeReader.leaf_field,i=b.p.treeReader.level_field,j=b.p.treeReader.parent_id_field,
k=b.p.treeReader.left_field,m=b.p.treeReader.right_field,n=b.p.treeReader.loaded,o,l,p,q,r;o=0;var s=e,t;void 0===f&&(f=!1);if(void 0===c||null===c){r=b.p.data.length-1;if(0<=r)for(;0<=r;)o=Math.max(o,parseInt(b.p.data[r][b.p.localReader.id],10)),r--;c=o+1}var v=a(b).jqGrid("getInd",e);t=!1;if(void 0===e||null===e||""===e)s=e=null,o="last",q=b.p.tree_root_level,r=b.p.data.length+1;else if(o="after",l=b.p._index[e],p=b.p.data[l],e=p[b.p.localReader.id],q=parseInt(p[i],10)+1,r=a(b).jqGrid("getFullTreeNode",
p),r.length?(s=r=r[r.length-1][b.p.localReader.id],r=a(b).jqGrid("getInd",s)+1):r=a(b).jqGrid("getInd",e)+1,p[g])t=!0,p[h]=!0,a(b.rows[v]).find("span.cell-wrapperleaf").removeClass("cell-wrapperleaf").addClass("cell-wrapper").end().find("div.tree-leaf").removeClass(b.p.treeIcons.leaf+" tree-leaf").addClass(b.p.treeIcons.minus+" tree-minus"),b.p.data[l][g]=!1,p[n]=!0;l=r+1;void 0===d[h]&&(d[h]=!1);void 0===d[n]&&(d[n]=!1);d[i]=q;void 0===d[g]&&(d[g]=!0);"adjacency"===b.p.treeGridModel&&(d[j]=e);if("nested"===
b.p.treeGridModel){var w;if(null!==e){g=parseInt(p[m],10);i=a.jgrid.from(b.p.data);i=i.greaterOrEquals(m,g,{stype:"integer"});i=i.select();if(i.length)for(w in i)i.hasOwnProperty(w)&&(i[w][k]=i[w][k]>g?parseInt(i[w][k],10)+2:i[w][k],i[w][m]=i[w][m]>=g?parseInt(i[w][m],10)+2:i[w][m]);d[k]=g;d[m]=g+1}else{g=parseInt(a(b).jqGrid("getCol",m,!1,"max"),10);i=a.jgrid.from(b.p.data).greater(k,g,{stype:"integer"}).select();if(i.length)for(w in i)i.hasOwnProperty(w)&&(i[w][k]=parseInt(i[w][k],10)+2);i=a.jgrid.from(b.p.data).greater(m,
g,{stype:"integer"}).select();if(i.length)for(w in i)i.hasOwnProperty(w)&&(i[w][m]=parseInt(i[w][m],10)+2);d[k]=g+1;d[m]=g+2}}if(null===e||a(b).jqGrid("isNodeLoaded",p)||t)a(b).jqGrid("addRowData",c,d,o,s),a(b).jqGrid("setTreeNode",r,l);p&&!p[h]&&f&&a(b.rows[v]).find("div.treeclick").click()}}})})(jQuery);
...
d.name){n[c]=a;break}});var l=0,p=a.makeArray(b.groupSummary);p.reverse();a.each(b.groups,function(q,r){l++;j=f.p.id+"ghead_"+r.idx;i=j+"_"+q;g="<span style='cursor:pointer;' class='ui-icon "+k+"' onclick=\"jQuery('#"+a.jgrid.jqID(f.p.id)+"').jqGrid('groupingToggle','"+i+"');return false;\"></span>";try{a.isArray(b.formatDisplayField)&&a.isFunction(b.formatDisplayField[r.idx])&&(r.displayValue=b.formatDisplayField[r.idx].call(f,r.displayValue,r.value,f.p.colModel[n[r.idx]],r.idx,b)),m=f.formatter(i,
r.displayValue,n[r.idx],r.value)}catch(s){m=r.displayValue}h+='<tr id="'+i+'" role="row" class= "ui-widget-content jqgroup ui-row-'+f.p.direction+" "+j+'"><td style="padding-left:'+12*r.idx+'px;" colspan="'+e+'">'+g+a.jgrid.template(b.groupText[r.idx],m,r.cnt,r.summary)+"</td></tr>";if(o-1===r.idx){var t=b.groups[q+1],v,w,z=void 0!==t?b.groups[q+1].startRow:c.length;for(w=r.startRow;w<z;w++)h+=c[w].join("");var J;if(void 0!==t){for(J=0;J<b.groupField.length&&t.dataIndex!==b.groupField[J];J++);l=b.groupField.length-
J}for(t=0;t<l;t++)if(p[t]){w="";b.groupCollapse&&!b.showSummaryOnHide&&(w=' style="display:none;"');h+="<tr"+w+' jqfootlevel="'+(r.idx-t)+'" role="row" class="ui-widget-content jqfoot ui-row-'+f.p.direction+'">';w=d(q,t,b.groups);var x=f.p.colModel,B,F=w.cnt;for(v=0;v<e;v++){var u="<td "+f.formatCol(v,1,"")+">&#160;</td>",L="{0}";a.each(w.summary,function(){if(this.nm===x[v].name){x[v].summaryTpl&&(L=x[v].summaryTpl);"string"===typeof this.st&&"avg"===this.st.toLowerCase()&&this.v&&0<F&&(this.v/=
